...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log(
      `[run-simulator-heartbeats] Found ${dueSimulations?.length || 0} simulations due for readings`,
    );
      ...
      try {
        ...
        if (sim.door_cycle_enabled && sim.door_cycle_next_change_at) {
          ...
          if (doorChangeTime <= now) {
            ...
            const cycleSeconds =
              newDoorState === 'open' ? sim.door_cycle_open_seconds : sim.door_cycle_closed_seconds;
            ...
            doorChanges++;
            console.log(
              `[run-simulator-heartbeats] Door cycle: ${sim.unit?.name} -> ${newDoorState}`,
            );
          }
        }
        ...
        readingsInjected++;
        console.log(
          `[run-simulator-heartbeats] Injected reading for ${sim.unit?.name}: ${sim.current_temperature}°F`,
        );
      } catch (simError) {
    ...
    for (const sim of doorCycleOnly || []) {
      try {
        ...
        console.error(
          `[run-simulator-heartbeats] Error processing door cycle ${sim.id}:`,
          doorError,
        );
      }
    }
