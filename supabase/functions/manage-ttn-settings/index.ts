...
async function resetStuckSensors(
  supabase: any,
  ...
  try {
    ...
    if (fetchError) {
      console.error(
        `[manage-ttn-settings] [${requestId}] Error fetching stuck sensors:`,
        fetchError,
      );
      return;
    }
    ...
    console.log(
      `[manage-ttn-settings] [${requestId}] Resetting ${sensorsToReset.length} stuck sensors to 'unknown' state`,
    );
    ...
    if (updateError) {
      console.error(
        `[manage-ttn-settings] [${requestId}] Error resetting stuck sensors:`,
        updateError,
      );
    } else {
      console.log(
        `[manage-ttn-settings] [${requestId}] Successfully reset ${sensorsToReset.length} sensors`,
      );
    }
  } catch (err) {
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (userError || !user) {
      console.error(
        '[manage-ttn-settings] User verification failed:',
    ...
    const allowedRoles = isReadOnlyAction ? ['owner', 'admin', 'manager'] : ['owner', 'admin'];
    ...
    if (action === 'get_credentials') {
      console.log(
        `[manage-ttn-settings] [${requestId}] GET_CREDENTIALS for org: ${organizationId}`,
      );
      ...
      const orgApiResult = safeDecryptWithStatus(
        settings.ttn_org_api_key_encrypted,
        encryptionSalt,
      );
      const appApiResult = safeDecryptWithStatus(settings.ttn_api_key_encrypted, encryptionSalt);
      const webhookResult = safeDecryptWithStatus(
        settings.ttn_webhook_secret_encrypted,
        encryptionSalt,
      );
      ...
      const allStepsComplete =
        stepDetails.webhook_created === true &&
      ...
      if (allStepsComplete && provisioningStatus === 'failed') {
        console.log(
          `[manage-ttn-settings] [${requestId}] Auto-correcting stale status: all steps complete but status was 'failed'`,
        );
      ...
      return new Response(
        JSON.stringify({
          ...
          org_api_secret_last4:
            settings.ttn_org_api_key_last4 || settings.ttn_gateway_api_key_last4,
    ...
    if (action === 'retry_provisioning') {
      console.log(
        `[manage-ttn-settings] [${requestId}] RETRY_PROVISIONING for org: ${organizationId}`,
      );
      ...
      if (!current) {
        return new Response(JSON.stringify({ error: 'TTN not configured for this organization' }), {
          status: 400,
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        });
      }
      ...
      const provisionResult = await fetch(`${supabaseUrl}/functions/v1/ttn-provision-org`, {
        method: 'POST',
        headers: {
    ...
    if (action === 'regenerate_all') {
      ...
      if (!isDevOrg) {
        ...
        console.log(
          `[manage-ttn-settings] [${requestId}] Rate limit bypassed for dev org: ${organizationId}`,
        );
      }
    ...
    if (action === 'test') {
      ...
      console.log(
        `[manage-ttn-settings] [${requestId}] Testing TTN connection for org: ${organizationId}`,
      );
      ...
      console.log(
        `[manage-ttn-settings] [${requestId}] Test result: success=${testResult.success}, error=${testResult.error || 'none'}`,
      );
      ...
      return new Response(JSON.stringify(enhancedTestResult), {
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
    ...
    if (action === 'validate_cluster') {
      console.log(
        `[manage-ttn-settings] [${requestId}] VALIDATE_CLUSTER for org: ${organizationId}`,
      );
      ...
      try {
        const appResponse = await fetch(`${TTN_BASE_URL}/api/v3/applications/${appId}`, {
      ...
      try {
        const webhookResponse = await fetch(`${TTN_BASE_URL}/api/v3/as/webhooks/${appId}`, {
          headers: { Authorization: `Bearer ${apiKey}` },
        });
        checks.webhook_on_nam1 = webhookResponse.ok;
        console.log(
          `[manage-ttn-settings] [${requestId}] Webhook check: ${webhookResponse.status}`,
        );
      } catch (err) {
    ...
    if (action === 'update') {
      console.log(
        `[manage-ttn-settings] [${requestId}] Updating settings for org: ${organizationId}`,
      );
      ...
      if (updates.is_enabled === true) {
        ...
        const hasCredentials =
          existingCheck?.ttn_application_id && existingCheck?.ttn_api_key_encrypted;
        if (!hasCredentials) {
          return new Response(
            JSON.stringify({
      ...
      const eventType = updates.api_key
        ...
        : updates.is_enabled !== undefined
          ? updates.is_enabled
            ? 'ttn.settings.enabled'
            : 'ttn.settings.disabled'
      ...
      await supabaseAdmin.from('event_logs').insert({
        ...
        title: updates.api_key
          ...
          : updates.is_enabled !== undefined
            ? updates.is_enabled
              ? 'TTN Integration Enabled'
              : 'TTN Integration Disabled'
            : 'TTN Settings Updated',
        actor_id: user.id,
        event_data: {
