...
Deno.serve(async (req) => {
  ...
  try {
    ...
    log('info', 'Processing webhook update request', {
      organization_id,
      user_id: user.id,
      requestId,
    });
      ...
      return new Response(
        JSON.stringify({
          ...
          error: {
            ...
            hint: 'Contact your organization owner',
          },
        }),
      ...
      return new Response(
        JSON.stringify({
          ...
          error: {
            ...
            hint: 'Set up TTN connection first',
          },
        }),
    ...
    const newEvents = enabled_events ??
      ttnConfig.ttn_webhook_events ?? ['uplink_message', 'join_accept'];
    ...
    if (
      ...
      JSON.stringify(enabled_events) !== JSON.stringify(ttnConfig.ttn_webhook_events)
    ) {
      changes.push({
        field: 'enabled_events',
        from: ttnConfig.ttn_webhook_events,
        to: enabled_events,
      });
    }
      ...
      return new Response(
        JSON.stringify({
          ...
          error: {
            ...
            message: 'TTN application ID not configured',
            hint: 'Complete TTN setup first',
          },
        }),
        { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } },
      );
      ...
      return new Response(
        JSON.stringify({
          ...
          error: {
            ...
            hint: 'Re-enter your TTN API key',
          },
        }),
    ...
    if (updateError) {
      log('error', 'Failed to update FrostGuard database', {
    ...
    await adminClient.from('org_sync_state').upsert(
      {
        organization_id,
        is_dirty: true,
