...
const TELNYX_ERROR_CODES: Record<string, string> = {
  ...
  '40013':
    'Invalid messaging source number - phone number may not be associated with messaging profile',
  ...
  try {
    ...
    console.log('send-sms-alert: Credentials check:', {
      ...
      messagingProfileId: TELNYX_MESSAGING_PROFILE_ID
        ? `${TELNYX_MESSAGING_PROFILE_ID.slice(0, 8)}...`
        : 'MISSING',
    ...
    if (TELNYX_PHONE_NUMBER === OLD_LONG_CODE) {
      ...
      return new Response(
        JSON.stringify({
          error:
            'SMS configuration error: Old long code detected. Update TELNYX_PHONE_NUMBER to toll-free number.',
          status: 'failed',
    ...
    if (!to || !message || !alertType || !organizationId) {
      ...
      return new Response(
        JSON.stringify({
          error: 'Missing required fields: to, message, alertType, organizationId',
        }),
    ...
    if (userId) {
      ...
      if (rateLimitError) {
        ...
        console.log(
          `send-sms-alert: Rate limited - SMS already sent to user ${userId} for ${alertType} within 15 minutes`,
        );
    ...
    try {
      ...
      if (verificationResponse.ok) {
        ...
        console.warn(
          `send-sms-alert: Could not check verification status: ${verificationResponse.status}`,
        );
    ...
    if (TELNYX_MESSAGING_PROFILE_ID) {
      console.log(
        `send-sms-alert: Using messaging profile: ${TELNYX_MESSAGING_PROFILE_ID.slice(0, 8)}... (frost guard)`,
      );
    }
    ...
    console.log(
      'send-sms-alert: Telnyx response data:',
      JSON.stringify({
    ...
    if (!telnyxResponse.ok || telnyxData.errors?.length) {
      ...
      const fullError = errorCode
        ? `${errorCode}: ${friendlyError}${errorDetail ? ` - ${errorDetail}` : ''}`
        : friendlyError;
      ...
      if (errorCode === '40013') {
        console.error(
          JSON.stringify({
            ...
            action:
              'Verify TELNYX_PHONE_NUMBER matches toll-free number +18889890560 and is associated with messaging profile in Telnyx portal',
            timestamp: new Date().toISOString(),
          }),
        );
      }
      ...
      return new Response(JSON.stringify({ error: fullError, status: 'failed', code: errorCode }), {
