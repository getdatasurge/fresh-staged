...
const PERMANENT_ERROR_CODES = ['INVALID_ORG', 'ORG_NOT_FOUND', 'TTN_ADMIN_NOT_CONFIGURED'];
...
serve(async (req) => {
  ...
  try {
    ...
    for (let i = 0; i < maxJobs; i++) {
      ...
      const { data: jobs, error: jobError } = await supabase.rpc('get_next_tts_provisioning_job');
      ...
      console.log(
        `[ttn-provision-worker] Processing job ${job.id} for org ${job.organization_id} (attempt ${job.attempts})`,
      );
      jobsProcessed++;
      ...
      try {
        ...
        const provisionResponse = await fetch(`${supabaseUrl}/functions/v1/ttn-provision-org`, {
          method: 'POST',
          headers: {
        ...
        console.log(
          `[ttn-provision-worker] Provision result for ${job.organization_id}:`,
          provisionResult,
        );
    ...
    console.log(
      `[ttn-provision-worker] Completed: ${jobsProcessed} processed, ${jobsSucceeded} succeeded, ${jobsFailed} failed`,
    );
