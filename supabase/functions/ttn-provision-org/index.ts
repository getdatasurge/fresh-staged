...
async function getValidWebhookFormats(
  ...
  try {
    ...
    if (!response.ok) {
      console.log(
        `[ttn-provision-org] [${requestId}] Failed to fetch webhook formats: ${response.status}`,
      );
    ...
    console.log(
      `[ttn-provision-org] [${requestId}] Available webhook formats: ${formats.join(', ')}`,
    );
    return formats;
  } catch (err) {
  ...
  try {
    ...
    if (!ttnAdminKey) {
      return buildResponse({
        ...
        message:
          'Add the Main User API Key (Personal API Key with full rights) as a secret in the project settings',
        request_id: requestId,
        retryable: false,
      });
    }
    ...
    console.log(
      `[ttn-provision-org] [${requestId}] Action: ${action}, Org: ${organization_id}, Region: ${region}`,
    );
    ...
    if (!roleCheck || !['owner', 'admin'].includes(roleCheck.role)) {
      return new Response(
        JSON.stringify({
          error: 'Only admins and owners can manage TTN provisioning',
          request_id: requestId,
        }),
    ...
    if (action === 'deep_clean') {
      console.log(
        `[ttn-provision-org] [${requestId}] Deep Clean: Starting complete TTN resource cleanup`,
      );
      ...
      if (currentAppId) {
        try {
          ...
          console.log(
            `[ttn-provision-org] [${requestId}] Deep Clean: Listing devices from ${listDevicesUrl}`,
          );
          ...
          if (listResponse.ok) {
            ...
            console.log(
              `[ttn-provision-org] [${requestId}] Deep Clean: Found ${devices.length} devices to delete`,
            );
            ...
            for (const device of devices) {
              ...
              if (deviceId) {
                ...
                if (deleteResponse.ok || deleteResponse.status === 404) {
                  deletedDevices++;
                  console.log(
                    `[ttn-provision-org] [${requestId}] Deep Clean: Deleted device ${deviceId}`,
                  );
                } else {
                  console.log(
                    `[ttn-provision-org] [${requestId}] Deep Clean: Failed to delete device ${deviceId}: ${deleteResponse.status}`,
                  );
                }
              }
            }
            console.log(
              `[ttn-provision-org] [${requestId}] Deep Clean: Deleted ${deletedDevices} devices from ${currentAppId}`,
            );
          } else {
            console.log(
              `[ttn-provision-org] [${requestId}] Deep Clean: Could not list devices (${listResponse.status}) - continuing`,
            );
          }
        } catch (err) {
          console.log(
            `[ttn-provision-org] [${requestId}] Deep Clean: Device deletion error (continuing): ${err}`,
          );
        }
      }
      ...
      if (currentAppId) {
        try {
          ...
          console.log(
            `[ttn-provision-org] [${requestId}] Deep Clean: Deleting application at ${deleteAppUrl}`,
          );
          ...
          if (deleteAppResponse.ok || deleteAppResponse.status === 404) {
            deletedApp = true;
            console.log(
              `[ttn-provision-org] [${requestId}] Deep Clean: Deleted application ${currentAppId}`,
            );
          } else {
            const errorText = await deleteAppResponse.text();
            console.log(
              `[ttn-provision-org] [${requestId}] Deep Clean: App deletion failed (${deleteAppResponse.status}): ${errorText}`,
            );
          }
        } catch (err) {
          console.log(
            `[ttn-provision-org] [${requestId}] Deep Clean: App deletion error (continuing): ${err}`,
          );
        }
      }
      ...
      if (currentOrgId) {
        try {
          ...
          console.log(
            `[ttn-provision-org] [${requestId}] Deep Clean: Deleting organization at ${deleteOrgUrl}`,
          );
          ...
          if (deleteOrgResponse.ok || deleteOrgResponse.status === 404) {
            deletedOrg = true;
            console.log(
              `[ttn-provision-org] [${requestId}] Deep Clean: Deleted organization ${currentOrgId}`,
            );
          } else {
            const errorText = await deleteOrgResponse.text();
            console.log(
              `[ttn-provision-org] [${requestId}] Deep Clean: Org deletion failed (${deleteOrgResponse.status}): ${errorText}`,
            );
          }
        } catch (err) {
          console.log(
            `[ttn-provision-org] [${requestId}] Deep Clean: Org deletion error (continuing): ${err}`,
          );
        }
      }
      ...
      if (clearError) {
        console.error(
          `[ttn-provision-org] [${requestId}] Deep Clean: DB clear error: ${clearError.message}`,
        );
      }
      ...
      if (sensorClearError) {
        console.log(
          `[ttn-provision-org] [${requestId}] Deep Clean: Sensor clear warning: ${sensorClearError.message}`,
        );
      }
      ...
      return buildResponse({
        ...
        request_id: requestId,
        next_step:
          "Use 'Start Provisioning' to create new TTN organization and application on NAM1",
      });
    }
    ...
    if (action === 'provision' || action === 'retry' || action === 'start_fresh') {
      ...
      if (action === 'retry' && ttnConn?.app_rights_check_status === 'forbidden') {
        console.log(
          `[ttn-provision-org] [${requestId}] Retry blocked: app is unowned, must use start_fresh`,
        );
        return buildResponse({
          success: false,
      ...
      if (action === 'start_fresh') {
        console.log(
          `[ttn-provision-org] [${requestId}] Start Fresh: UNCONDITIONALLY clearing ALL TTN credentials`,
        );
        ...
        if (clearError) {
          console.error(
            `[ttn-provision-org] [${requestId}] Start Fresh: Failed to clear credentials: ${clearError.message}`,
          );
        } else {
          console.log(
            `[ttn-provision-org] [${requestId}] Start Fresh: ALL TTN credentials cleared from database`,
          );
        }
        ...
        console.log(
          `[ttn-provision-org] [${requestId}] Start Fresh: Refreshed ttnConn - ttn_application_id is now: ${ttnConn?.ttn_application_id}`,
        );
      }
      ...
      if (action === 'start_fresh' || action === 'retry') {
        ...
        console.log(
          `[ttn-provision-org] [${requestId}] Start Fresh/Retry: Generated collision-safe app ID: ${ttnAppId}`,
        );
      } else {
        ttnAppId = ttnConn?.ttn_application_id || generateTtnApplicationId(org.id);
        console.log(
          `[ttn-provision-org] [${requestId}] ttnAppId assigned: ${ttnAppId} (from DB: ${ttnConn?.ttn_application_id || 'null - generated fresh'})`,
        );
      }

      console.log(
        `[ttn-provision-org] [${requestId}] Provisioning TTN org: ${ttnOrgId}, app: ${ttnAppId} for org ${org.slug}${appIdRotated ? ' (rotated)' : ''}`,
      );
      console.log(
        `[ttn-provision-org] [${requestId}] ALL STEPS will use token_source: ${TOKEN_SOURCE}`,
      );
      ...
      if (action === 'start_fresh') {
        completedSteps = {};
        console.log(
          `[ttn-provision-org] [${requestId}] Start Fresh: Reset completedSteps to force all steps to run`,
        );
        ...
        if (previousAppId && previousAppId !== ttnAppId) {
          console.log(
            `[ttn-provision-org] [${requestId}] Retry: App ID changed from "${previousAppId}" to "${ttnAppId}" - resetting app-scoped steps`,
          );
      ...
      if (completedSteps.org_api_key_created && !ttnConn?.ttn_org_api_key_encrypted) {
        console.log(
          `[ttn-provision-org] [${requestId}] CONSISTENCY REPAIR: org_api_key_created=true but ttn_org_api_key_encrypted is NULL - resetting flag`,
        );
        await logProvisioningStep(
          supabase,
          organization_id,
      ...
      try {
        ...
        console.log(
          `[ttn-provision-org] [${requestId}] Step 0: Cached preflight_user_id="${cachedPreflightUserId || 'none'}", forcing fresh preflight`,
        );
        ...
        // Always run preflight - never skip based on preflight_done
        {
          ...
          console.log(
            `[ttn-provision-org] [${requestId}] Step 0: Preflight check (token_source: ${TOKEN_SOURCE})`,
          );
          ...
          if (!preflightResult.success) {
            ...
            await logProvisioningStep(
              ...
              duration,
              requestId,
              preflightResult.statusCode,
              preflightResult.hint,
              'authentication',
              ttnEndpoint,
            );
          ...
          if (cachedPreflightUserId && cachedPreflightUserId !== preflightResult.user_id) {
            console.log(
              `[ttn-provision-org] [${requestId}] Step 0: User ID CHANGED: "${cachedPreflightUserId}" -> "${preflightResult.user_id}"`,
            );
            await logProvisioningStep(
              supabase,
              organization_id,
              'preflight_user_changed',
              'success',
        ...
        orgCreationLoop: while (
          !completedSteps.organization_created ||
          !completedSteps.organization_verified
        ) {
          ...
          if (!preflightUserId || preflightUserId === 'unknown') {
            ...
            return buildResponse({
              success: false,
              error:
                'Preflight did not return a valid TTN user_id. Check that TTN_ADMIN_API_KEY is a Personal API Key.',
          ...
          console.log(
            `[ttn-provision-org] [${requestId}] Step 1: Using preflight_user_id="${preflightUserId}" for org creation endpoint`,
          );
          console.log(
            `[ttn-provision-org] [${requestId}] Step 1: Org ID attempt: "${effectiveTtnOrgId}" (rotation ${orgRotationAttempts}/${maxOrgRotationAttempts})`,
          );
          ...
          console.log(
            `[ttn-provision-org] [${requestId}] Step 1: Creating TTN organization ${effectiveTtnOrgId}`,
          );
          console.log(`[ttn-provision-org] [${requestId}] Step 1: Endpoint: ${ttnEndpoint}`);
          console.log(
            `[ttn-provision-org] [${requestId}] Step 1: Payload: ${JSON.stringify(createPayload)}`,
          );

          let createOrgResponse: Response;
          let createResponseText = '';
          try {
            createOrgResponse = await fetchWithTimeout(ttnEndpoint, {
              method: 'POST',
              headers: {
            ...
            console.log(
              `[ttn-provision-org] [${requestId}] Step 1: Response status=${createOrgResponse.status}, body=${createResponseText.slice(0, 500)}`,
            );
          } catch (fetchErr) {
          ...
          if (
            createOrgResponse.status !== 200 &&
            createOrgResponse.status !== 201 &&
            createOrgResponse.status !== 409
          ) {
            const duration = Date.now() - step1Start;
          ...
          console.log(
            `[ttn-provision-org] [${requestId}] Step 1: Org ${wasCreated ? 'created (200/201)' : 'exists (409)'}, proceeding to verification`,
          );

          // ============ STEP 1 VERIFICATION: Verify org exists and we have rights ============
          console.log(
            `[ttn-provision-org] [${requestId}] Step 1 Verification: Checking org ${effectiveTtnOrgId}`,
          );
          ...
          if (!verifyOrgResponse.ok) {
            ...
            if (verifyOrgResponse.status === 404 || verifyOrgResponse.status === 403) {
              ...
              if (orgRotationAttempts < maxOrgRotationAttempts) {
                ...
                console.log(
                  `[ttn-provision-org] [${requestId}] ORG COLLISION DETECTED: "${oldOrgId}" exists but no access (${verifyOrgResponse.status})`,
                );
                console.log(
                  `[ttn-provision-org] [${requestId}] Rotating org ID: "${oldOrgId}" -> "${effectiveTtnOrgId}" (attempt ${orgRotationAttempts}/${maxOrgRotationAttempts})`,
                );
              ...
              await logProvisioningStep(
                ...
                {
                  status: verifyOrgResponse.status,
                  response: verifyText.slice(0, 500),
                  rotation_attempts: orgRotationAttempts,
                },
                duration,
                requestId,
                verifyOrgResponse.status,
                verifyText,
                undefined,
                verifyOrgEndpoint,
              );
          ...
          try {
            ...
            await logProvisioningStep(
              ...
              'verify_organization',
              'failed',
              `Failed to verify org rights: ${errMsg}`,
              {},
              duration,
              requestId,
            );
          ...
          if (!verifyRightsResponse.ok) {
            ...
            await logProvisioningStep(
              ...
              { status: verifyRightsResponse.status },
              duration,
              requestId,
              verifyRightsResponse.status,
              rightsText,
            );
          ...
          const hasOrgApiKeyRight = orgRights.some(
            (r) => r === 'RIGHT_ORGANIZATION_SETTINGS_API_KEYS' || r === 'RIGHT_ORGANIZATION_ALL',
          );
          ...
          console.log(
            `[ttn-provision-org] [${requestId}] Step 1: Organization ${effectiveTtnOrgId} verified successfully${orgRotationAttempts > 0 ? ` (after ${orgRotationAttempts} ID rotation(s))` : ''}`,
          );
        ...
        if (completedSteps.organization_created && completedSteps.organization_verified) {
          console.log(
            `[ttn-provision-org] [${requestId}] Step 1: Skipping (org already created and verified)`,
          );
        ...
        if (!completedSteps.org_api_key_created) {
          ...
          await logProvisioningStep(
            ...
            { endpoint: ttnEndpoint, org_id: effectiveTtnOrgId },
            undefined,
            requestId,
            undefined,
            undefined,
          ...
          console.log(
            `[ttn-provision-org] [${requestId}] Step 1B: Creating org-scoped API key for ${effectiveTtnOrgId} (token_source: ${TOKEN_SOURCE})`,
          );

          let createOrgKeyResponse: Response;
          try {
            createOrgKeyResponse = await fetchWithTimeout(ttnEndpoint, {
              method: 'POST',
              headers: {
                Authorization: `Bearer ${ttnAdminKey}`,
                'Content-Type': 'application/json',
          ...
          if (createOrgKeyResponse.status === 409) {
            console.log(
              `[ttn-provision-org] [${requestId}] Step 1B: API key name conflict (409), retrying with unique name`,
            );
            ...
            try {
              ...
              console.log(
                `[ttn-provision-org] [${requestId}] Step 1B: Retry with unique name result: ${createOrgKeyResponse.status}`,
              );
            } catch (retryErr) {
              const errMsg = retryErr instanceof Error ? retryErr.message : 'Retry fetch failed';
              console.error(
                `[ttn-provision-org] [${requestId}] Step 1B: Retry with unique name failed: ${errMsg}`,
              );
            }
          }
          ...
          if (!createOrgKeyResponse.ok) {
            ...
            await logProvisioningStep(
              ...
              { status: createOrgKeyResponse.status },
              duration,
              requestId,
              createOrgKeyResponse.status,
              errorText,
              category,
              ttnEndpoint,
            );
          ...
          console.log(
            `[ttn-provision-org] [${requestId}] Step 1B DEBUG: TTN response keys: ${JSON.stringify(Object.keys(orgKeyData))}`,
          );
          console.log(
            `[ttn-provision-org] [${requestId}] Step 1B DEBUG: key value type: ${typeof orgApiKey}, length: ${orgApiKey?.length || 0}`,
          );
          console.log(
            `[ttn-provision-org] [${requestId}] Step 1B DEBUG: key prefix (safe): ${orgApiKey?.substring(0, 6) || 'EMPTY'}`,
          );
          ...
          if (!orgApiKey && orgKeyData.api_key) {
            const nestedKey =
              typeof orgKeyData.api_key === 'string' ? orgKeyData.api_key : orgKeyData.api_key?.key;
            if (nestedKey && typeof nestedKey === 'string') {
              console.log(
                `[ttn-provision-org] [${requestId}] Step 1B: Found key in nested api_key field`,
              );
              orgApiKey = nestedKey;
            }
          }
          ...
          if (orgApiKey && !orgApiKey.startsWith('NNSXS.')) {
            console.warn(
              `[ttn-provision-org] [${requestId}] Step 1B WARNING: Key doesn't start with NNSXS. prefix: ${orgApiKey.substring(0, 10)}`,
            );
          }
          ...
          if (!orgApiKey) {
            const duration = Date.now() - step1bStart;
            console.error(
              `[ttn-provision-org] [${requestId}] Step 1B: No key value found. Full response: ${JSON.stringify(orgKeyData)}`,
            );
            await logProvisioningStep(
              supabase,
              organization_id,
              ...
              { response_keys: Object.keys(orgKeyData) },
              duration,
              requestId,
            );
          ...
          console.log(
            `[ttn-provision-org] [${requestId}] Step 1B: Round-trip check - original_last4: ${originalLast4}, decrypted_last4: ${decryptedLast4}, match: ${originalLast4 === decryptedLast4}`,
          );
          console.log(
            `[ttn-provision-org] [${requestId}] Step 1B: Original length: ${orgApiKey.length}, encrypted length: ${encryptedOrgKey.length}, decrypted length: ${decryptedVerify.length}`,
          );

          if (orgApiKey !== decryptedVerify) {
            console.error(
              `[ttn-provision-org] [${requestId}] Step 1B: ENCRYPTION CORRUPTION DETECTED!`,
            );
            console.error(
              `[ttn-provision-org] [${requestId}] Step 1B: Original prefix: ${orgApiKey.substring(0, 10)}, Decrypted prefix: ${decryptedVerify.substring(0, 10)}`,
            );
          }
          ...
          if (persistError) {
            const duration = Date.now() - step1bStart;
            console.error(
              `[ttn-provision-org] [${requestId}] Step 1B: FAILED to persist org API key:`,
              persistError,
            );
            await logProvisioningStep(
              supabase,
          ...
          await logProvisioningStep(
            ...
            { key_last4: getLast4(orgApiKey), key_id: orgApiKeyId },
            duration,
            requestId,
            createOrgKeyResponse.status,
            undefined,
          ...
          console.log(
            `[ttn-provision-org] [${requestId}] Step 1B: Org API key created and persisted (key_last4: ${getLast4(orgApiKey)})`,
          );
        } else {
          ...
          if (!keyCheck?.ttn_org_api_key_encrypted) {
            console.log(
              `[ttn-provision-org] [${requestId}] Step 1B: Flag says created but key missing - re-running step`,
            );
            completedSteps.org_api_key_created = false;
          ...
          console.log(
            `[ttn-provision-org] [${requestId}] Step 1B: Skipping (org key already created, key_last4: ${keyCheck.ttn_org_api_key_last4})`,
          );
        }
        ...
        if (!completedSteps.application_created) {
          ...
          await logProvisioningStep(
            ...
            { ttn_app_id: ttnAppId, ttn_org_id: effectiveTtnOrgId, endpoint: ttnEndpoint },
            undefined,
            requestId,
            undefined,
            undefined,
          ...
          console.log(
            `[ttn-provision-org] [${requestId}] Step 2: Creating application ${ttnAppId} under org ${effectiveTtnOrgId} (token_source: ${TOKEN_SOURCE})`,
          );

          let createAppResponse: Response;
          try {
            createAppResponse = await fetchWithTimeout(ttnEndpoint, {
              method: 'POST',
              headers: {
                Authorization: `Bearer ${ttnAdminKey}`,
                'Content-Type': 'application/json',
          ...
          if (!createAppResponse.ok && createAppResponse.status !== 409) {
            ...
            await logProvisioningStep(
              ...
              'create_application',
              'failed',
              `HTTP ${createAppResponse.status}`,
              { status: createAppResponse.status },
              duration,
              requestId,
              createAppResponse.status,
              errorText,
              category,
              ttnEndpoint,
            );
          ...
          await logProvisioningStep(
            ...
            { ttn_app_id: ttnAppId, ttn_org_id: ttnOrgId },
            duration,
            requestId,
            createAppResponse.status,
            undefined,
        ...
        if (!completedSteps.app_rights_verified) {
          ...
          await logProvisioningStep(
            supabase,
            organization_id,
            'verify_application_rights',
            'started',
            `Verifying application rights (token_source: ${TOKEN_SOURCE})`,
            { ttn_app_id: ttnAppId, endpoint: ttnEndpoint },
            undefined,
            requestId,
            undefined,
            undefined,
          ...
          console.log(
            `[ttn-provision-org] [${requestId}] Step 2B: Verifying rights for application ${ttnAppId}`,
          );

          let rightsResponse: Response;
          try {
            rightsResponse = await fetchWithTimeout(ttnEndpoint, {
              method: 'GET',
              headers: { Authorization: `Bearer ${ttnAdminKey}` },
            });
          } catch (fetchErr) {
          ...
          if (rightsResponse.status === 403) {
            ...
            await logProvisioningStep(
              ...
              { status: 403, error_name: ttnError.name },
              duration,
              requestId,
              403,
              rightsText,
            ...
            return buildResponse({
              ...
              error: 'Application exists but current key has no rights',
              message:
                "This TTN application exists but the current provisioning key has no rights to it. This commonly happens with legacy apps created under another account. Use 'Start Fresh' to recreate or generate a new app ID under the current key.",
              step: 'verify_application_rights',
              retryable: false,
              use_start_fresh: true,
              request_id: requestId,
          ...
          if (rightsResponse.status === 404) {
            ...
            return buildResponse({
              ...
              message:
                "The TTN application was not found. It may have been deleted from the TTN console. Use 'Start Fresh' to recreate it.",
              step: 'verify_application_rights',
              retryable: false,
              use_start_fresh: true,
              request_id: requestId,
          ...
          if (!rightsResponse.ok) {
            ...
            await logProvisioningStep(
              ...
              'verify_application_rights',
              'failed',
              `HTTP ${rightsResponse.status}`,
              { status: rightsResponse.status },
              duration,
              requestId,
              rightsResponse.status,
              rightsText,
              'unknown',
              ttnEndpoint,
            );
          ...
          console.log(
            `[ttn-provision-org] [${requestId}] Step 2B: Rights verified for ${ttnAppId}`,
          );
        } else {
          console.log(
            `[ttn-provision-org] [${requestId}] Step 2B: Skipping (rights already verified)`,
          );
        }
        ...
        if (completedSteps.org_api_key_created) {
          ...
          if (ttnConnRefresh?.ttn_org_api_key_encrypted) {
            orgApiKeyForAppOps = deobfuscateKey(
              ttnConnRefresh.ttn_org_api_key_encrypted,
              encryptionSalt,
            );
            console.log(
              `[ttn-provision-org] [${requestId}] Retrieved Org API Key for Steps 3-4 (key_last4: ${getLast4(orgApiKeyForAppOps)})`,
            );

            // DEBUG: Verify key format after decryption
            console.log(
              `[ttn-provision-org] [${requestId}] Step 3 DEBUG: Decrypted key length: ${orgApiKeyForAppOps.length}`,
            );
            console.log(
              `[ttn-provision-org] [${requestId}] Step 3 DEBUG: Decrypted key prefix: ${orgApiKeyForAppOps.substring(0, 6)}`,
            );

            if (!orgApiKeyForAppOps.startsWith('NNSXS.')) {
              console.error(
                `[ttn-provision-org] [${requestId}] Step 3: DECRYPTION ISSUE - key doesn't start with NNSXS.`,
              );
              console.error(
                `[ttn-provision-org] [${requestId}] Step 3: encrypted_length: ${ttnConnRefresh.ttn_org_api_key_encrypted.length}, decrypted_length: ${orgApiKeyForAppOps.length}`,
              );
            }
          }
        }
        ...
        if (!orgApiKeyForAppOps) {
          console.error(
            `[ttn-provision-org] [${requestId}] Org API Key not found - required for app operations`,
          );
          return buildResponse({
            success: false,
            error:
              "Org API Key not found - required for app operations. Try 'Start Fresh' to re-provision.",
        ...
        if (completedSteps.app_api_key_created) {
          ...
          if (storedAppId !== ttnAppId) {
            console.log(
              `[ttn-provision-org] [${requestId}] APP ID MISMATCH: Stored App API Key is for "${storedAppId}", but current app is "${ttnAppId}". Forcing recreation.`,
            );
            completedSteps.app_api_key_created = false;
            ...
            appApiKeyForWebhook = deobfuscateKey(
              ttnConnForAppKey.ttn_api_key_encrypted,
              encryptionSalt,
            );
            console.log(
              `[ttn-provision-org] [${requestId}] Retrieved App API Key for Step 4 (key_last4: ${getLast4(appApiKeyForWebhook)}, app_id: ${storedAppId})`,
            );
          }
        }
        ...
        if (!completedSteps.app_api_key_created) {
          ...
          console.log(
            `[ttn-provision-org] [${requestId}] Step 3: Creating app-scoped API key for ${ttnAppId} (token_source: org_api_key)`,
          );

          let createAppKeyResponse: Response;
          try {
            createAppKeyResponse = await fetchWithTimeout(ttnEndpoint, {
              method: 'POST',
              headers: {
          ...
          if (!createAppKeyResponse.ok) {
            ...
            await logProvisioningStep(
              ...
              'create_app_api_key',
              'failed',
              `HTTP ${createAppKeyResponse.status}`,
              {
                status: createAppKeyResponse.status,
              },
              duration,
              requestId,
              createAppKeyResponse.status,
              errorText,
              category,
              ttnEndpoint,
            );
          ...
          console.log(
            `[ttn-provision-org] [${requestId}] Step 3: Round-trip check - original_last4: ${originalAppLast4}, decrypted_last4: ${decryptedAppLast4}, match: ${originalAppLast4 === decryptedAppLast4}`,
          );
          console.log(
            `[ttn-provision-org] [${requestId}] Step 3: Original length: ${appApiKey.length}, encrypted length: ${encryptedAppKey.length}, decrypted length: ${decryptedAppKeyVerify.length}`,
          );

          if (appApiKey !== decryptedAppKeyVerify) {
            console.error(
              `[ttn-provision-org] [${requestId}] Step 3: APP KEY ENCRYPTION CORRUPTION DETECTED!`,
            );
            console.error(
              `[ttn-provision-org] [${requestId}] Step 3: Original prefix: ${appApiKey.substring(0, 10)}, Decrypted prefix: ${decryptedAppKeyVerify.substring(0, 10)}`,
            );
            ...
            await logProvisioningStep(
              ...
              { original_last4: originalAppLast4, decrypted_last4: decryptedAppLast4 },
              duration,
              requestId,
            );
            ...
            return buildResponse({
              success: false,
              error:
                "App API key was corrupted during encryption. Please click 'Retry' to recreate.",
              step: 'create_app_api_key',
              retryable: true,
              request_id: requestId,
            });
          ...
          if (!appApiKey.startsWith('NNSXS.')) {
            console.warn(
              `[ttn-provision-org] [${requestId}] Step 3 WARNING: App key doesn't start with NNSXS. prefix`,
            );
          }
          ...
          await logProvisioningStep(
            ...
            'create_app_api_key',
            'success',
            'Application API key created [output artifact]',
            { key_last4: originalAppLast4, roundtrip_verified: true },
            duration,
            requestId,
            createAppKeyResponse.status,
            undefined,
          ...
          console.log(
            `[ttn-provision-org] [${requestId}] Step 3: Captured App API Key for Step 4 (key_last4: ${originalAppLast4})`,
          );
          ...
          console.log(
            `[ttn-provision-org] [${requestId}] Step 3: Skipping (app key already created)`,
          );
        }
        ...
        if (!completedSteps.gateway_key_attempted) {
          ...
          console.log(
            `[ttn-provision-org] [${requestId}] Step 3B: Creating gateway API key for org ${effectiveTtnOrgId} (token_source: org_api_key)`,
          );
          ...
          try {
            ...
            if (createGatewayKeyResponse.ok) {
              ...
              console.log(
                `[ttn-provision-org] [${requestId}] Step 3B: Round-trip check - original_last4: ${originalGwLast4}, decrypted_last4: ${decryptedGwLast4}, match: ${originalGwLast4 === decryptedGwLast4}`,
              );

              if (gatewayApiKey !== decryptedGwKeyVerify) {
                console.error(
                  `[ttn-provision-org] [${requestId}] Step 3B: GATEWAY KEY ENCRYPTION CORRUPTION DETECTED!`,
                );
                ...
                await logProvisioningStep(
                  ...
                  { original_last4: originalGwLast4, decrypted_last4: decryptedGwLast4 },
                  duration,
                  requestId,
                );
              } else {
                ...
                await logProvisioningStep(
                  ...
                  'create_gateway_key',
                  'success',
                  'Gateway API key created [output artifact]',
                  { key_last4: originalGwLast4, roundtrip_verified: true },
                  duration,
                  requestId,
                );
              ...
              console.log(
                `[ttn-provision-org] [${requestId}] Step 3B: Gateway key creation returned ${createGatewayKeyResponse.status}, continuing...`,
              );
              const duration = Date.now() - step3bStart;
              await logProvisioningStep(
                supabase,
                ...
                'create_gateway_key',
                'skipped',
                `Gateway key creation skipped (${createGatewayKeyResponse.status}) - not critical`,
                { status: createGatewayKeyResponse.status },
                duration,
                requestId,
              );
            }
            ...
            await logProvisioningStep(
              ...
              'create_gateway_key',
              'skipped',
              `Gateway key creation failed (${errMsg}) - not critical`,
              { error: errMsg },
              duration,
              requestId,
            );
            console.log(
              `[ttn-provision-org] [${requestId}] Step 3B: Gateway key creation failed (not critical): ${errMsg}`,
            );
          }
        ...
        if (!completedSteps.webhook_created) {
          ...
          if (!orgApiKeyForAppOps) {
            ...
            if (ttnConnForOrgKey?.ttn_org_api_key_encrypted) {
              orgApiKeyForAppOps = deobfuscateKey(
                ttnConnForOrgKey.ttn_org_api_key_encrypted,
                encryptionSalt,
              );
              console.log(
                `[ttn-provision-org] [${requestId}] Step 4: Retrieved Org API Key from DB (key_last4: ${getLast4(orgApiKeyForAppOps)})`,
              );
            }
          }
          ...
          if (!orgApiKeyForAppOps) {
            console.error(
              `[ttn-provision-org] [${requestId}] Step 4: Org API Key not available for webhook creation`,
            );
            return buildResponse({
              success: false,
              error:
                "Org API Key not found - required for webhook creation. Try 'Start Fresh' to re-provision.",
              step: 'create_webhook',
              retryable: true,
              request_id: requestId,
            });
          ...
          await logProvisioningStep(
            ...
            { webhook_url: webhookUrl, endpoint: ttnEndpoint },
            undefined,
            requestId,
            undefined,
            undefined,
          ...
          console.log(
            `[ttn-provision-org] [${requestId}] Step 4: Creating webhook for ${ttnAppId} via PUT (token_source: org_api_key, key_last4: ${getLast4(orgApiKeyForAppOps)})`,
          );
          ...
          if (!webhookUrl || typeof webhookUrl !== 'string' || !webhookUrl.startsWith('https://')) {
            console.error(
              `[ttn-provision-org] [${requestId}] CRITICAL: Invalid webhookUrl at Step 4: "${webhookUrl}" (type: ${typeof webhookUrl})`,
            );
            return buildResponse({
              success: false,
          ...
          const webhookPayload = {
            ...
            field_mask: {
              paths: ['format', 'base_url', 'headers', 'uplink_message'],
            },
          };

          // DEBUG: Log exact payload being sent
          console.log(
            `[ttn-provision-org] [${requestId}] Step 4: base_url value: "${webhookUrl}" (type: ${typeof webhookUrl})`,
          );
          console.log(
            `[ttn-provision-org] [${requestId}] Step 4: Webhook payload (PUT method):`,
          ...
          try {
            ...
            createWebhookResponse = await fetchWithTimeout(ttnEndpoint, {
              method: 'PUT',
              headers: {
                Authorization: `Bearer ${orgApiKeyForAppOps}`,
                'Content-Type': 'application/json',
          ...
          if (!createWebhookResponse.ok) {
            ...
            await logProvisioningStep(
              ...
              'create_webhook',
              'failed',
              `HTTP ${createWebhookResponse.status}`,
              { status: createWebhookResponse.status },
              duration,
              requestId,
              createWebhookResponse.status,
              errorText,
              category,
              ttnEndpoint,
            );
          ...
          console.log(
            `[ttn-provision-org] [${requestId}] Step 4: Skipping (webhook already created)`,
          );
        }
        ...
        await logProvisioningStep(
          ...
          { ttn_org_id: ttnOrgId, ttn_app_id: ttnAppId },
          undefined,
          requestId,
        );
        ...
        console.log(
          `[ttn-provision-org] [${requestId}] Provisioning complete! All steps used token_source: ${TOKEN_SOURCE}`,
        );
        ...
        const errMsg =
          provisionError instanceof Error ? provisionError.message : 'Unknown provisioning error';
    ...
    if (action === 'delete') {
      ...
      console.log(
        `[ttn-provision-org] [${requestId}] Deleting TTN resources: org=${ttnOrgId}, app=${ttnAppId} (token_source: ${TOKEN_SOURCE})`,
      );
      ...
      if (ttnAppId) {
        try {
          ...
          console.log(
            `[ttn-provision-org] [${requestId}] Delete app response: ${deleteAppResponse.status}`,
          );
        } catch (err) {
      ...
      try {
        ...
        console.log(
          `[ttn-provision-org] [${requestId}] Delete org response: ${deleteOrgResponse.status}`,
        );
      } catch (err) {
    ...
    if (action === 'regenerate_webhook_secret') {
      ...
      console.log(
        `[ttn-provision-org] [${requestId}] Regenerating webhook secret for ${ttnAppId} (token_source: ${TOKEN_SOURCE})`,
      );
