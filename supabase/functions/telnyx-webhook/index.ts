...
async function verifyTelnyxSignature(
  ...
  try {
    ...
    if (publicKeyBytes.length !== 32) {
      console.error(
        `telnyx-webhook: Invalid public key length: ${publicKeyBytes.length} bytes (expected 32)`,
      );
      console.error(
        'telnyx-webhook: Ensure TELNYX_PUBLIC_KEY is the Ed25519 public key from Telnyx API settings',
      );
      return false;
    }
    ...
    console.error(
      'telnyx-webhook: Key format hint - ensure TELNYX_PUBLIC_KEY is the hex or base64 Ed25519 public key from Telnyx API settings',
    );
    return false;
  }
}
...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    if (TELNYX_PUBLIC_KEY) {
      ...
      if (!isValid) {
        if (ALLOW_UNSIGNED_WEBHOOKS) {
          console.warn(
            'telnyx-webhook: Signature invalid but ALLOW_UNSIGNED_WEBHOOKS=true, proceeding',
          );
        } else {
          console.error('telnyx-webhook: Invalid signature - rejecting webhook');
          console.error(
            'telnyx-webhook: To debug, temporarily set ALLOW_UNSIGNED_WEBHOOKS=true or verify TELNYX_PUBLIC_KEY',
          );
      ...
      if (ALLOW_UNSIGNED_WEBHOOKS) {
        console.warn(
          'telnyx-webhook: No public key configured, ALLOW_UNSIGNED_WEBHOOKS=true, proceeding',
        );
      } else {
    ...
    const messagingProfileId = (payload.data?.payload as Record<string, unknown>)
      ?.messaging_profile_id as string | undefined;
    ...
    if (messagingProfileId) {
      console.log(
        'telnyx-webhook: Messaging Profile:',
        messagingProfileId.slice(0, 8) + '... (frost guard)',
      );
    }
    ...
    const { error: insertEventError } = await supabase.from('telnyx_webhook_events').insert({
      event_id: eventId,
      event_type: eventType,
      message_id: messageId,
      payload: payload,
    ...
    if (messageId && mappedStatus && (mappedStatus === 'delivered' || mappedStatus === 'failed')) {
      ...
      if (mappedStatus === 'failed') {
        ...
        if (errors?.length) {
          errorMessage = errors
            .map((e) => `${e.code}: ${e.title}${e.detail ? ` - ${e.detail}` : ''}`)
            .join('; ');
        } else {
    ...
    return new Response(JSON.stringify({ status: 'ok', processed: true, event_id: eventId }), {
      status: 200,
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
    });
  } catch (error: unknown) {
