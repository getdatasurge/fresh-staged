...
serve(async (req) => {
  ...
  try {
    ...
    console.log(
      `[ttn-manage-application] Action: ${action}, Org: ${organization_id}, App: ${ttnAppId}`,
    );
    ...
    if (action === 'verify' || action === 'ensure') {
      ...
      const checkResponse = await fetch(`${baseUrl}/api/v3/applications/${ttnAppId}`, {
        method: 'GET',
        headers: {
    ...
    if (action === 'configure_webhook' || action === 'ensure') {
      if (!org.ttn_webhook_configured) {
        ...
        const webhookResponse = await fetch(`${baseUrl}/api/v3/as/webhooks/${ttnAppId}`, {
          method: 'POST',
          headers: {
            Authorization: `Bearer ${ttnConfig.apiKey}`,
            'Content-Type': 'application/json',
