...
serve(async (req) => {
  ...
  if (!authResult.valid) {
    log('warn', 'TTN_SETTINGS_SYNC_AUTH_FAILED', {
      request_id: requestId,
    ...
    return errorResponse(
      ...
      'Provide valid PROJECT2_SYNC_API_KEY',
      requestId,
    );
  }
  ...
  try {
    ...
    return errorResponse(
      ...
      'INVALID_JSON',
      'Invalid JSON body',
      'Provide valid JSON request body',
      requestId,
    );
  }
  ...
  try {
    ...
    if (orgError || !org) {
      log('warn', 'TTN_SETTINGS_SYNC_ORG_NOT_FOUND', {
        request_id: requestId,
        org_id: body.org_id,
      });
      return errorResponse(
    ...
    const { data: ttnConn, error: upsertError } = await supabase
      ...
      .select(
        'is_enabled, ttn_region, ttn_application_id, ttn_api_key_last4, ttn_api_key_updated_at, ttn_webhook_secret_last4, ttn_webhook_url, ttn_webhook_id, ttn_webhook_events, ttn_last_updated_source, updated_at',
      )
      .single();
    ...
    await supabase.from('org_sync_state').upsert(
      {
        organization_id: body.org_id,
        is_dirty: true,
