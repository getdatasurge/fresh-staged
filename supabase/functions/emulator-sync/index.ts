...
const corsHeaders = {
  ...
  'Access-Control-Allow-Headers':
    'authorization, x-client-info, apikey, content-type, x-emulator-sync-key',
};
...
Deno.serve(async (req) => {
  ...
  if (contentLength && parseInt(contentLength) > 1024 * 1024) {
    ...
    return new Response(JSON.stringify({ error: 'Payload too large', max_bytes: 1048576 }), {
      status: 413,
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
    });
  }
  ...
  console.log(
    `[emulator-sync] Validated payload for org: ${payload.org_id}, gateways: ${payload.gateways.length}, devices: ${payload.devices.length}, sensors: ${payload.sensors.length}`,
  );
      ...
      if (device.dev_eui) {
        try {
          ...
          const sensorData: EmulatorSensorInput = {
            ...
            decoded_payload:
              (device as { decoded_payload?: Record<string, unknown> }).decoded_payload || null,
          };
          ...
          console.log(
            `[emulator-sync] Auto-sensor from device ${device.serial_number}: explicit_type=${device.sensor_type || 'none'}, model=${device.model || 'none'}, has_payload=${!!(device as { decoded_payload?: Record<string, unknown> }).decoded_payload}`,
          );

          const sensorResult = await upsertSensor(supabase, payload.org_id, sensorData);
          counts.sensors[sensorResult]++;
          console.log(
            `[emulator-sync] Auto-created sensor for device ${device.serial_number}: ${sensorResult}`,
          );
        } catch (sensorErr) {
  ...
  const { error: syncRunError } = await supabase.from('emulator_sync_runs').insert(syncRunData);
...
async function upsertGateway(
  ...
  if (existing) {
    // Update existing
    const { error } = await supabase.from('gateways').update(upsertData).eq('id', existing.id);

    if (error) throw new Error(error.message);
    return 'updated';
  } else {
    // Insert new
    const { error } = await supabase.from('gateways').insert(upsertData);
...
async function upsertDevice(
  ...
  if (existing) {
    // Update existing
    const { error } = await supabase.from('devices').update(upsertData).eq('id', existing.id);

    if (error) throw new Error(error.message);
    return 'updated';
  } else {
    // Insert new
    const { error } = await supabase.from('devices').insert(upsertData);
...
async function upsertSensor(
  ...
  if (existing) {
    ...
    const { error } = await supabase.from('lora_sensors').update(upsertData).eq('id', existing.id);

    if (error) throw new Error(error.message);

    console.log(
      `[emulator-sync] Updated sensor ${sensor.dev_eui}: status=${finalStatus} (was ${existingStatus}), has_app_key=${!!upsertData.app_key || !!existing.app_key}`,
    );
    return 'updated';
  } else {
    // Insert new
    const { error } = await supabase.from('lora_sensors').insert(upsertData);

    if (error) throw new Error(error.message);

    console.log(
      `[emulator-sync] Created sensor ${sensor.dev_eui}: status=${finalStatus}, has_app_key=${!!upsertData.app_key}`,
    );
    return 'created';
  }
}
