...
Deno.serve(async (req) => {
  ...
  try {
    ...
    switch (action) {
      case 'pair_sensor': {
        ...
        const configData = {
          ...
          current_temperature: generateNormalTemp(
            unitData.temp_limit_high,
            unitData.temp_limit_low,
          ),
          created_by: userId,
        };
        ...
        await logEvent(supabase, organizationId!, unit_id, userId, 'pair_sensor', {
          serial_number: serialNumber,
        });
        break;
      }
      ...
      case 'set_online': {
        if (!simConfig || !simConfig.sensor_paired) {
          return new Response(JSON.stringify({ error: 'Sensor must be paired first' }), {
            status: 400,
            headers: { ...corsHeaders, 'Content-Type': 'application/json' },
          });
        }
      ...
      case 'set_offline': {
        if (!simConfig) {
          return new Response(JSON.stringify({ error: 'No simulation config found' }), {
            status: 400,
            headers: { ...corsHeaders, 'Content-Type': 'application/json' },
          });
        }
      ...
      case 'update_telemetry': {
        ...
        if (body.door_sensor_present !== undefined)
          updates.door_sensor_present = body.door_sensor_present;
      ...
      case 'set_door_state': {
        ...
        await logEvent(supabase, organizationId!, unit_id, userId, 'set_door_state', {
          door_state: newDoorState,
        });
      ...
      case 'inject': {
        if (!simConfig) {
          ...
          const { data: newConfig } = await supabase
            ...
            .insert({
              ...
              current_temperature:
                body.temperature ??
                generateNormalTemp(unitData.temp_limit_high, unitData.temp_limit_low),
              created_by: userId,
            })
            .select()
            .single();
        ...
        await logEvent(supabase, organizationId!, unit_id, userId, 'inject', {
          temperature: simConfig.current_temperature,
        });
      ...
      case 'start_streaming': {
        ...
        await injectReading(
          supabase,
          { ...simConfig, streaming_enabled: true, sensor_online: true } as SimulatedDeviceConfig,
          unitData,
        );

        result.message = `Streaming started - readings every ${interval}s`;
        result.interval_seconds = interval;
        await logEvent(supabase, organizationId!, unit_id, userId, 'start_streaming', {
      ...
      case 'start': {
        ...
        await logEvent(supabase, organizationId!, unit_id, userId, 'start', {
    ...
    return new Response(JSON.stringify({ success: true, ...result, config: finalConfig }), {
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
    });
  } catch (error: any) {
...
async function injectReading(
  ...
  if (config.door_sensor_present) {
    ...
    const { data: insertedEvent, error: doorError } = await supabase
      .from('door_events')
      .insert({
        unit_id: config.unit_id,
  ...
  console.log(
    `[sensor-simulator] Injected reading: unit=${config.unit_id}, temp=${config.current_temperature}°F, door=${config.door_state}`,
  );
}
