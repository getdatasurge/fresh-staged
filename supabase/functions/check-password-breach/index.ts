...
serve(async (req) => {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return new Response(JSON.stringify({ breached: false, count: 0 }), {
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
    ...
    return new Response(JSON.stringify({ breached, count: breachCount }), {
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
    });
  } catch (error) {
    console.error('Error checking password breach:', error);
    return new Response(JSON.stringify({ error: 'Failed to check password' }), {
      status: 500,
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
    });
  }
