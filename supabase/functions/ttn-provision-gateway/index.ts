...
serve(async (req) => {
  ...
  try {
    ...
    console.log(
      `[ttn-provision-gateway] [${requestId}] Action: ${action}, Gateway: ${gateway_id}, Org: ${organization_id}`,
    );
    ...
    if (action === 'create') {
      ...
      if (checkResponse.status !== 404) {
        ...
        console.error(
          `[ttn-provision-gateway] [${requestId}] Check failed: ${checkResponse.status} - ${errorText}`,
        );

        if (checkResponse.status === 403 || checkResponse.status === 401) {
          // API key doesn't have gateway read rights - continue and try to create
          console.log(
            `[ttn-provision-gateway] [${requestId}] Cannot check gateway existence (${checkResponse.status}), proceeding to create`,
          );
        } else {
          await supabase
            .from('gateways')
      ...
      if (authInfoResponse.ok) {
        ...
        console.log(
          `[ttn-provision-gateway] [${requestId}] Auth info:`,
          JSON.stringify({
        ...
        console.warn(
          `[ttn-provision-gateway] [${requestId}] Could not get auth_info: ${authInfoResponse.status}`,
        );
      }
      ...
      for (const strategy of strategies) {
        ...
        if (createResponse.ok) {
          console.log(
            `[ttn-provision-gateway] [${requestId}] Gateway registered via ${strategy.name}`,
          );
        ...
        console.error(
          `[ttn-provision-gateway] [${requestId}] Strategy ${strategy.name} failed: ${createResponse.status} - ${errorText}`,
        );
    ...
    if (action === 'delete') {
      console.log(
        `[ttn-provision-gateway] [${requestId}] Deleting gateway ${ttnGatewayId} from TTN`,
      );
