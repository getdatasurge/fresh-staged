...
Deno.serve(async (req) => {
  ...
  try {
    ...
    for (const unit of (units || []) as unknown[]) {
      ...
      const doorLastChanged = u.door_last_changed_at
        ? new Date(u.door_last_changed_at as string).getTime()
        : null;
      const doorOpenDuration =
        doorState === 'open' && doorLastChanged ? Math.floor((now - doorLastChanged) / 60000) : 0;
      ...
      const { data: effectiveRules } = await supabase.rpc('get_effective_alert_rules', {
        p_unit_id: u.id,
      });
      ...
      const offlineSeverity = computeOfflineSeverity(
        missedCheckins,
        offlineWarningThreshold,
        offlineCriticalThreshold,
      );
      ...
      if (missedCheckins === 0 && offlineSeverity === 'none') {
        if (
          ...
          currentStatus === 'monitoring_interrupted' ||
      ...
      if (missedCheckins === 0 && u.last_temp_reading !== null) {
        ...
        if (isAboveLimit || isBelowLimit) {
          ...
          if (isDoorOpenWithinGrace && currentStatus === 'ok') {
            console.log(
              `Unit ${u.name}: Temp out of range but door open (${doorOpenDuration}/${doorGraceMinutes}m grace)`,
            );
            ...
            const statusChangeTime = u.last_status_change
              ? new Date(u.last_status_change as string).getTime()
              : now;
            ...
            if (timeInExcursion >= confirmTime) {
              ...
              console.log(
                `Unit ${u.name}: Excursion pending confirmation (${Math.floor(timeInExcursion / 60000)}/${Math.floor(confirmTime / 60000)}m)`,
              );
            }
          }
          ...
          if (
            (doorState === 'closed' || doorState === 'unknown') &&
            (currentStatus === 'excursion' ||
              currentStatus === 'alarm_active' ||
              newStatus === 'excursion' ||
              newStatus === 'alarm_active')
          ) {
            const coolingCheckTime = new Date(
              now - COOLING_FAILURE_THRESHOLD_MINUTES * 60 * 1000,
            ).toISOString();
          ...
          const inRangeWithHysteresis =
            temp <= highLimit - hysteresis && (lowLimit === null || temp >= lowLimit + hysteresis);
          ...
          if (inRangeWithHysteresis) {
            if (currentStatus === 'excursion' || currentStatus === 'alarm_active') {
              ...
              const allInRange =
                (recentReadings || []).length >= READINGS_TO_RESTORE &&
                (recentReadings || []).every((r: { temperature: number }) => {
                  const t = r.temperature;
                  return (
                    t <= highLimit - hysteresis && (lowLimit === null || t >= lowLimit + hysteresis)
                  );
                });
      ...
      if (newStatus !== currentStatus) {
        ...
        if (['monitoring_interrupted', 'manual_required', 'offline'].includes(newStatus)) {
          ...
          if (!existingAlert) {
            ...
            if (alertData) {
              newAlertIds.push(alertData.id);
              console.log(
                `Created monitoring_interrupted alert (${alertSeverity}) for unit ${u.name}`,
              );
            }
            ...
            console.log(
              `Updated monitoring_interrupted alert to ${alertSeverity} for unit ${u.name}`,
            );
          }
        }
        ...
        await supabase.from('event_logs').insert({
          ...
          severity:
            newStatus === 'alarm_active'
              ? 'error'
              : newStatus === 'excursion' || newStatus === 'manual_required'
                ? 'warning'
                : 'info',
    ...
    console.log(
      `Process complete: ${stateChanges.length} state changes, ${newAlertIds.length} new alerts`,
    );
