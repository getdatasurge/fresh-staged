...
serve(async (req) => {
  ...
  try {
    ...
    console.log(
      `[ttn-provision-device] Action: ${action}, Sensor: ${sensor_id}, Org: ${organization_id}`,
    );
    ...
    console.log(
      `[ttn-provision-device] Region: ${ttnConfig.region}, Frequency plan: ${frequencyPlan}`,
    );
    ...
    if (action === 'create') {
      ...
      const probeResponse = await ttnFetch(
      ...
      if (!probeResponse.ok) {
        ...
        console.error(
          `[ttn-provision-device] TTN probe failed: ${probeResponse.status} - ${probeErrorText}`,
        );
      ...
      if (getResponse.status === 404) {
        ...
        if (!createResponse.ok) {
          ...
          if (createResponse.status === 409) {
            ...
            if (
              errorText.includes('end_device_euis_taken') ||
              ...
              if (existingApp !== ttnAppId) {
                console.error(
                  `[ttn-provision-device] Cross-app conflict: DevEUI in ${existingApp}`,
                );
      ...
      if (!verifyResponse.ok) {
        ...
        console.error(
          `[ttn-provision-device] Verification failed: ${verifyResponse.status} - ${errorText}`,
        );
        ...
        console.log(
          `[ttn-provision-device] Device verified in ${ttnConfig.region?.toUpperCase()} cluster`,
        );
      }
    ...
    if (action === 'delete') {
      ...
      return new Response(JSON.stringify({ success: true, message: 'Device deleted from TTN' }), {
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
    ...
    if (action === 'diagnose') {
      ...
      const isCheck = await ttnFetch(
        `/api/v3/applications/${ttnAppId}/devices/${deviceId}`,
        { method: 'GET' },
        'diagnose_is',
      );
      checks.is = { ok: isCheck.ok, status: isCheck.status };
      if (!isCheck.ok && isCheck.status !== 404)
        checks.is.error = (await isCheck.text()).substring(0, 200);

      // Step 4: Check JS (Join Server - has root keys)
      const jsCheck = await ttnFetch(
        `/api/v3/js/applications/${ttnAppId}/devices/${deviceId}`,
        { method: 'GET' },
        'diagnose_js',
      );
      checks.js = { ok: jsCheck.ok, status: jsCheck.status };
      if (!jsCheck.ok && jsCheck.status !== 404)
        checks.js.error = (await jsCheck.text()).substring(0, 200);

      // Step 5: Check NS (Network Server)
      const nsCheck = await ttnFetch(
        `/api/v3/ns/applications/${ttnAppId}/devices/${deviceId}`,
        { method: 'GET' },
        'diagnose_ns',
      );
      checks.ns = { ok: nsCheck.ok, status: nsCheck.status };
      if (!nsCheck.ok && nsCheck.status !== 404)
        checks.ns.error = (await nsCheck.text()).substring(0, 200);

      // Step 6: Check AS (Application Server)
      const asCheck = await ttnFetch(
        `/api/v3/as/applications/${ttnAppId}/devices/${deviceId}`,
        { method: 'GET' },
        'diagnose_as',
      );
      checks.as = { ok: asCheck.ok, status: asCheck.status };
      if (!asCheck.ok && asCheck.status !== 404)
        checks.as.error = (await asCheck.text()).substring(0, 200);
      ...
      if (isExists && jsExists && nsExists && asExists) {
        ...
        hint =
          'Device exists in Identity Server but missing from Join Server. Keys may not be set. Try re-provisioning.';
      } else if (!isExists && (jsExists || nsExists || asExists)) {
        diagnosis = 'split_brain_orphaned';
        hint =
          'Device missing from Identity Server but exists in other planes. Orphaned state - may need manual cleanup in TTN Console.';
      } else {
        diagnosis = 'partial';
