...
export const GATEWAY_RIGHTS = {
  required: ['RIGHT_GATEWAY_INFO', 'RIGHT_GATEWAY_SETTINGS_BASIC', 'RIGHT_GATEWAY_LINK'],
  extended: ['RIGHT_GATEWAY_STATUS_READ', 'RIGHT_GATEWAY_LOCATION_READ', 'RIGHT_GATEWAY_DELETE'],
...
// ============================================================================
export const APPLICATION_KEY_RIGHTS_ALL = ['RIGHT_APPLICATION_ALL'];
...
// ============================================================================
export const GATEWAY_KEY_RIGHTS_ALL = ['RIGHT_GATEWAY_ALL'];
...
export async function validateMainUserApiKey(
  ...
  try {
    ...
    console.log(
      `[ttnPermissions] [${requestId}] DEBUG raw auth_info (truncated): ${JSON.stringify(data).substring(0, 1500)}`,
    );
    console.log(
      `[ttnPermissions] [${requestId}] Preflight: raw response keys: ${Object.keys(data).join(', ')}`,
    );
    ...
    console.log(
      `[ttnPermissions] [${requestId}] DEBUG entity_ids raw: ${JSON.stringify(outer?.entity_ids)}`,
    );
    console.log(
      `[ttnPermissions] [${requestId}] DEBUG user_ids raw: ${JSON.stringify(outer?.entity_ids?.user_ids)}`,
    );
    ...
    console.log(
      `[ttnPermissions] [${requestId}] Preflight: outer keys: ${Object.keys(outer || {}).join(', ')}`,
    );
    console.log(
      `[ttnPermissions] [${requestId}] Preflight: inner exists: ${!!inner}, rights count: ${rights.length}`,
    );
    console.log(
      `[ttnPermissions] [${requestId}] Preflight: entity_type=${entityType}, user_id=${userId}, is_admin=${isAdmin}`,
    );
    ...
    if (entityType === 'application_ids') {
      ...
      console.log(
        `[ttnPermissions] [${requestId}] Preflight: Rejected - Application-scoped key for ${appId}`,
      );
      return {
        success: false,
        user_id: userId,
    ...
    if (entityType === 'organization_ids') {
      ...
      console.log(
        `[ttnPermissions] [${requestId}] Preflight: Rejected - Organization-scoped key for ${orgId}`,
      );
      return {
        success: false,
        user_id: userId,
    ...
    if (entityType === 'gateway_ids') {
      ...
      console.log(
        `[ttnPermissions] [${requestId}] Preflight: Rejected - Gateway-scoped key for ${gwId}`,
      );
      return {
        success: false,
        user_id: userId,
    ...
    console.log(
      `[ttnPermissions] [${requestId}] Preflight: ${allRights.length} total rights, ${missingRights.length} missing`,
    );

    // Check if key has sufficient rights
    if (missingRights.length > 0) {
      console.log(
        `[ttnPermissions] [${requestId}] Preflight: Missing rights: ${missingRights.join(', ')}`,
      );
      return {
        success: false,
        user_id: userId,
        ...
        hint: `The key is valid but missing these rights: ${missingRights
          .slice(0, 3)
          .map((r) => PERMISSION_LABELS[r] || r)
          .join(
            ', ',
          )}${missingRights.length > 3 ? ` and ${missingRights.length - 3} more` : ''}. Edit the key in TTN Console or create a new one with 'Grant all current and future rights' checked.`,
      };
    }

    // Success - key is valid and has sufficient rights
    console.log(
      `[ttnPermissions] [${requestId}] Preflight: SUCCESS - Valid Personal API key with all required rights`,
    );
    return {
      success: true,
      user_id: userId,
...
export async function fetchTtnRights(
  ...
  try {
    ...
    if (!response.ok) {
      const errorText = await response.text();
      console.error(
        `[ttnPermissions] [${requestId}] Rights fetch failed: ${response.status} ${errorText}`,
      );
    ...
    console.log(
      `[ttnPermissions] [${requestId}] Fetched ${rights.length} rights from TTN:`,
      rights,
    );
