...
export function getClusterBaseUrl(region: string | null | undefined): string {
  ...
  if (requested !== 'nam1') {
    console.warn(
      `[getClusterBaseUrl] NAM1-ONLY: Region "${requested}" requested but NAM1 enforced`,
    );
  }
  return TTN_BASE_URL;
}
...
export async function lookupOrgByWebhookSecret(
  ...
  if (error || !data) {
    console.log(
      `[lookupOrgByWebhookSecret] No match for secret (error: ${error?.message || 'no data'})`,
    );
    return null;
  }
...
export async function getTtnConfigForOrg(
  ...
  if (requestedRegion !== 'nam1') {
    console.warn(
      `[getTtnConfigForOrg] NAM1-ONLY: Stored region "${requestedRegion}" overridden to "nam1"`,
    );
  }
...
export async function testTtnConnection(
  ...
  try {
    ...
    const appResponse = await fetch(`${baseUrl}${appEndpoint}`, {
      method: 'GET',
      headers: {
    ...
    if (!appResponse.ok) {
      ...
      console.error(
        `[testTtnConnection] Identity Server app test failed: ${appResponse.status} ${errorText}`,
      );
    ...
    if (options?.testDeviceId) {
      ...
      const deviceResponse = await fetch(`${baseUrl}${deviceEndpoint}`, {
        method: 'GET',
        headers: {
          Authorization: `Bearer ${config.apiKey}`,
          Accept: 'application/json',
