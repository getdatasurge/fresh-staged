...
export const sensorSourceSchema = z.enum(['ttn', 'ble', 'simulator', 'manual_sensor', 'api']);
...
export const reportTypeSchema = z.enum(['daily', 'exceptions', 'manual', 'compliance']);
...
export const emulatorSyncPayloadSchema = z.object({
  ...
  gateways: z
    .array(emulatorGatewaySchema)
    .max(50, 'Maximum 50 gateways per sync')
    .optional()
    .default([]),
  devices: z
    .array(emulatorDeviceSchema)
    .max(100, 'Maximum 100 devices per sync')
    .optional()
    .default([]),
  sensors: z
    .array(emulatorSensorSchema)
    .max(100, 'Maximum 100 sensors per sync')
    .optional()
    .default([]),
});
