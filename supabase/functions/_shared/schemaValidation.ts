 ...
 */
function validateField(value: unknown, schema: FieldSchema, path: string): ValidationError | null {
  // Type validation
 ...
 */
export function findMatchingSchemas(payload: Record<string, unknown>): SchemaMatch[] {
  const payloadKeys = Object.keys(payload);
  const matches: SchemaMatch[] = [];

  for (const schema of PAYLOAD_SCHEMAS) {
    // Check if discriminator fields are present
    const hasDiscriminators = schema.discriminatorFields.every((field) => field in payload);
    ...
    if (result.valid) {
      ...
      const matchedFields = Object.keys(schema.fields).filter((f) => f in payload);
      const missingFields = schema.requiredFields.filter((f) => !(f in payload));
      ...
      const confidence = schemaFieldCount > 0 ? matchedFieldCount / schemaFieldCount : 0;
 ...
 */
export function getBestMatchingSchema(payload: Record<string, unknown>): {
