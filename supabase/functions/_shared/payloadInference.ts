 ...
 */
function checkMultiSensorPattern(payload: Record<string, unknown>): {
  matched: boolean;
  model: string | null;
} {
 ...
 */
function detectAmbiguity(matches: Array<{ key: string; entry: DiscriminatorEntry }>): {
  isAmbiguous: boolean;
  alternates: string[];
} {
  if (matches.length <= 1) {
  ...
  // Get top types
  const sortedTypes = [...typesByPriority.entries()].sort((a, b) => b[1] - a[1]);
  ...
  const alternates = isAmbiguous ? sortedTypes.slice(1).map(([type]) => type) : [];
...
export function inferPayloadType(
  ...
  const { minConfidence = 0.5, skipAmbiguityCheck = false, includeDetails = true } = options;
