...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log(
      `[check-slug-available] Request: slug="${slug}", exclude_org_id=${excludeOrgId || 'none'}`,
    );
    ...
    if (!normalizedSlug || normalizedSlug.length < 2) {
      console.log(
        `[check-slug-available] Rejected: slug too short (${normalizedSlug.length} chars)`,
      );
      return new Response(
        JSON.stringify({
          available: false,
    ...
    if (!isAvailable) {
      ...
      console.log(
        `[check-slug-available] Generated ${response.suggestions.length} suggestions: ${response.suggestions.join(', ')}`,
      );
    }

    console.log(
      `[check-slug-available] Result: slug="${normalizedSlug}", available=${isAvailable}, suggestions=${response.suggestions.length}`,
    );
