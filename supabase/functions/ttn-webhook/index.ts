...
import {
  normalizeDevEui,
  formatDevEuiForDisplay,
  lookupOrgByWebhookSecret,
...
const corsHeaders = {
  ...
  'Access-Control-Allow-Headers':
    'authorization, x-client-info, apikey, content-type, x-downlink-apikey, x-webhook-secret',
};
...
Deno.serve(async (req) => {
  ...
  console.log(
    `[TTN-WEBHOOK] ${requestId} | X-Webhook-Secret present: ${req.headers.has('x-webhook-secret')}`,
  );
  ...
  console.log(
    `[TTN-WEBHOOK] ${requestId} | Secret source: ${providedSecretFromHeader ? 'X-Webhook-Secret' : providedSecretFromApikey ? 'X-Downlink-Apikey' : 'NONE'}`,
  );
  ...
  console.log(
    `[TTN-WEBHOOK] ${requestId} | AUTH SUCCESS - Org: ${authenticatedOrgId}, App: ${authenticatedAppId}`,
  );
  ...
  try {
    ...
    if (!payload.uplink_message) {
      const eventType = payload.join_accept
        ? 'join_accept'
        : payload.downlink_ack
          ? 'downlink_ack'
          : payload.downlink_nack
            ? 'downlink_nack'
            : 'unknown';

      console.log(
        `[TTN-WEBHOOK] ${requestId} | Non-uplink event: ${eventType}, org: ${authenticatedOrgId}`,
      );
      return new Response(
    ...
    const receivedAt =
      ttnPayload.uplink_message?.received_at || ttnPayload.received_at || new Date().toISOString();

    console.log(
      `[TTN-WEBHOOK] ${requestId} | Uplink: device=${deviceId}, dev_eui=${rawDevEui}, app=${applicationId}`,
    );

    // Validate that the uplink comes from the expected TTN application
    if (authenticatedAppId && applicationId !== authenticatedAppId) {
      console.warn(
        `[TTN-WEBHOOK] ${requestId} | Application ID mismatch! Expected: ${authenticatedAppId}, Got: ${applicationId}`,
      );
    ...
    if (deviceId) {
      const { data: sensorByDeviceId } = await supabase
        .from('lora_sensors')
        .select(
          'id, organization_id, site_id, unit_id, dev_eui, status, ttn_application_id, sensor_type, name, is_primary, model',
        )
      ...
      if (sensorByDeviceId) {
        console.log(
          `[TTN-WEBHOOK] ${requestId} | Found sensor by ttn_device_id: ${sensorByDeviceId.name}`,
        );
    ...
    if (!loraSensor) {
      const { data: sensorByDevEui } = await supabase
        ...
        .or(
          `dev_eui.eq.${devEuiLower},dev_eui.eq.${devEuiUpper},dev_eui.eq.${devEuiColonUpper},dev_eui.eq.${devEuiColonLower}`,
        )
        .eq('organization_id', authenticatedOrgId) // CRITICAL: Scope to authenticated org
        .maybeSingle();
    ...
    const { data: device } = await supabase
      ...
      .or(
        `serial_number.eq.${devEuiLower},serial_number.eq.${devEuiUpper},serial_number.eq.${devEuiColonUpper},serial_number.eq.${devEuiColonLower}`,
      )
    ...
    // Unknown device
    console.warn(
      `[TTN-WEBHOOK] ${requestId} | Unknown device for org ${authenticatedOrgId}: ${devEuiLower}`,
    );
    return new Response(
      JSON.stringify({
        accepted: true,
...
async function handleLoraSensor(
  ...
  console.log(
    `[TTN-WEBHOOK] ${requestId} | Processing sensor: ${sensor.name}, temp: ${temperature ?? 'N/A'}`,
  );
  ...
  if (payloadInferredType && payloadInferredType !== currentType) {
    ...
    if (currentType === 'temperature' || !currentType) {
      ...
      console.log(
        `[TTN-WEBHOOK] ${requestId} | Correcting sensor type: ${currentType} -> ${payloadInferredType} (based on payload keys: ${Object.keys(decoded).join(', ')})`,
      );
    }
  }
  ...
  if (sensor.unit_id) {
    ...
    if (hasDoorData) {
      ...
      console.log(
        `[TTN-WEBHOOK] ${requestId} | Door data normalized: ${normalizedDoorOpen} from ${doorSource}`,
      );
    }

    if (!hasTemperatureData && !hasDoorData) {
      console.log(
        `[TTN-WEBHOOK] ${requestId} | No temp or door data in payload. Keys: ${Object.keys(decoded).join(', ')}`,
      );
      return new Response(
        JSON.stringify({
          success: true,
          message: 'Sensor updated, no data to record',
          sensor_id: sensor.id,
        }),
        { status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json' } },
      );
    ...
    const hasDoorCapability =
      hasDoorData || sensor.sensor_type === 'door' || sensor.sensor_type === 'combo';
    ...
    if (shouldProcessDoor) {
      ...
      const isInitialReading =
        previousDoorState === 'unknown' ||
        previousDoorState === null ||
        previousDoorState === undefined;
      ...
      if (isInitialReading || stateChanged) {
        ...
        if (doorEventError) {
          console.error(
            `[TTN-WEBHOOK] ${requestId} | Door event insert FAILED:`,
            doorEventError.message,
          );
        } else {
          console.log(
            `[TTN-WEBHOOK] ${requestId} | Door event: ${previousDoorState} -> ${newDoorState} (initial: ${isInitialReading})`,
          );
        }
      }
    }
    ...
    if (unitUpdateError) {
      ...
      console.log(
        `[TTN-WEBHOOK] ${requestId} | Unit updated: door_state=${unitUpdate.door_state || 'unchanged'}, temp=${unitUpdate.last_temp_reading || 'unchanged'}`,
      );
    }
  ...
  return new Response(
    JSON.stringify({
      success: true,
      message: 'Sensor updated (not assigned to unit)',
      sensor_id: sensor.id,
    }),
    { status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json' } },
  );
...
async function handleLegacyDevice(
  ...
  return new Response(JSON.stringify({ success: true, device_id: device.id }), {
    status: 200,
