  ...
  try {
    ...
    switch (action) {
      case 'configure': {
        ...
        const webhookId = existingConfig?.webhook_id;
        let telnyxResponse;
        ...
        if (webhookId) {
          ...
          const { error: updateError } = await supabase.from('telnyx_webhook_config').upsert(
            {
          ...
          return new Response(
            JSON.stringify({
              ...
              status: 'active',
              message:
                'Webhook configuration updated. Ensure your Telnyx messaging profile has the webhook URL configured.',
            }),
          ...
          const { error: insertError } = await supabase.from('telnyx_webhook_config').insert({
            organization_id: organization_id || null,
            webhook_url: webhookUrl,
          ...
          return new Response(
            JSON.stringify({
              ...
              status: 'active',
              message:
                'Webhook configuration created. Configure this URL in your Telnyx messaging profile.',
            }),
            { status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json' } },
          );
        }
