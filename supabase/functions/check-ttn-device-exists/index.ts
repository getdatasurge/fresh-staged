...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (sensorsError) {
      console.error(
        `[check-ttn-device-exists] [${requestId}] Error fetching sensors:`,
        sensorsError,
      );
    ...
    if (configError) {
      console.error(
        `[check-ttn-device-exists] [${requestId}] Error fetching TTN configs:`,
        configError,
      );
    }

    console.log(
      `[check-ttn-device-exists] [${requestId}] Found ${ttnConfigs?.length || 0} TTN configs`,
    );
    ...
    for (const [orgId, orgSensors] of sensorsByOrg.entries()) {
      ...
      console.log(
        `[check-ttn-device-exists] [${requestId}] Org ${orgId}: Listing devices from TTN app ${config.application_id}`,
      );
      ...
      try {
        ...
        if (listResponse.ok) {
          ...
          console.log(
            `[check-ttn-device-exists] [${requestId}] Org ${orgId}: Found ${ttnDeviceMap.size} devices with DevEUI in TTN (cluster: ${config.cluster})`,
          );
        } else {
        ...
        console.error(
          `[check-ttn-device-exists] [${requestId}] Org ${orgId}: List devices error:`,
          err,
        );
      }
      ...
      for (const sensor of orgSensors) {
        ...
        if (ttnDeviceId) {
          ...
          console.log(
            `[check-ttn-device-exists] [${requestId}] ${sensor.name}: exists_in_ttn (${ttnDeviceId})`,
          );
        } else {
...
async function updateSensor(
  ...
  const { error } = await supabase.from('lora_sensors').update(updates).eq('id', sensorId);
