  ...
  for (const endpoint of endpoints) {
    try {
      ...
      if (response.ok) {
        ...
        console.log(
          `[ttn-deprovision-worker] ${endpoint.name} returned 404 (not found/already deleted)`,
        );
        notFoundCount++;
      } else {
        const errorText = await response.text();
        console.log(
          `[ttn-deprovision-worker] ${endpoint.name} failed: ${response.status} - ${errorText}`,
        );
        lastError = errorText;
...
serve(async (req) => {
  ...
  try {
    ...
    for (const job of jobs as DeprovisionJob[]) {
      ...
      if (!targetAppId) {
        ...
        await logEvent(
          ...
          false,
          'NO_APP_ID',
          'No TTN Application ID available',
        );
      ...
      try {
        ...
        console.log(
          `[ttn-deprovision-worker] Deleting device ${deviceId} from app ${targetAppId} on cluster ${ttnConfig.clusterBaseUrl}`,
        );
        ...
        if (deleteResult.success) {
          ...
          await supabase
            ...
            .update({
              ...
              last_error_message: deleteResult.alreadyDeleted
                ? 'Device was already deleted from TTN'
                : null,
            })
            .eq('id', job.id);
          ...
          await supabase
            ...
            .update({
              ...
              completed_at:
                newStatus === 'FAILED' || newStatus === 'BLOCKED' ? new Date().toISOString() : null,
            })
            .eq('id', job.id);

          const eventType =
            newStatus === 'BLOCKED' ? 'blocked' : newStatus === 'FAILED' ? 'failed' : 'retrying';
          await logEvent(
            supabase,
            eventType,
            job,
          ...
          console.log(
            `[ttn-deprovision-worker] Job ${job.id} ${newStatus}: ${classification.error_message}`,
          );
        }
      } catch (error) {
    ...
    return new Response(JSON.stringify({ error: message, processed: 0 }), {
      status: 500,
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
    });
  }
