  ...
  try {
    ...
    if (site_id) {
      const { data: siteData } = await supabase
        .from('sites')
        .select(
          'id, name, timezone, compliance_mode, manual_log_cadence_seconds, corrective_action_required',
        )
        .eq('id', site_id)
        .maybeSingle();
      if (siteData) {
    ...
    const orgUnits = (units || []).filter(
      (u: any) =>
        u.area?.site?.organization_id === profile.organization_id &&
        (!site_id || u.area?.site?.id === site_id),
    );

    if (orgUnits.length === 0) {
      return new Response(JSON.stringify({ error: 'No units found' }), {
        status: 404,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
    ...
    const { data: alerts } = await supabase
      .from('alerts')
      .select(
        'unit_id, title, alert_type, severity, status, triggered_at, acknowledged_at, acknowledged_by, acknowledgment_notes, resolved_at, resolved_by, temp_reading, temp_limit',
      )
      .in('unit_id', unitIds)
    ...
    const acknowledgerIds = [
      ...new Set(
        (alerts || []).filter((a: any) => a.acknowledged_by).map((a: any) => a.acknowledged_by),
      ),
    ];
    ...
    if (report_type === 'daily') {
      ...
      csv =
        'Date,Time,Unit Name,Site,Area,Reading Type,Temperature (°F),In Range,Logged By,Notes\n';
      ...
      csv =
        'Date,Time,Unit Name,Site,Event Type,Details,Temperature (°F),Limit (°F),Severity,Status,Acknowledged By,Acknowledged At,Acknowledgment Notes,Action Taken\n';
      ...
      (alerts || []).forEach((a: any) => {
        ...
        const ackBy = a.acknowledged_by
          ? acknowledgerMap[a.acknowledged_by] || a.acknowledged_by
          : '';
      ...
      csv =
        'Section,Date,Time,Unit Name,Site,Area,Type,Temperature (°F),High Limit,Low Limit,In Range,Logged By,Alert Type,Severity,Status,Action Taken,Notes\n';
