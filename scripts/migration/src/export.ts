...
function getTablesToExport(options: ExportOptions): TableName[] {
  ...
  if (options.skipLarge) {
    return allTables.filter(
      (t) => !(LARGE_TABLES as readonly string[]).includes(t)
    ) as TableName[];
  }
...
async function dryRun(options: ExportOptions): Promise<void> {
  ...
  for (const table of tables) {
    try {
      ...
      const marker = isLarge ? " [LARGE]" : "";
      console.log(
        `  ${table.padEnd(25)} ${String(count).padStart(10)} rows  (${method})${marker}`
      );
    } catch (error) {
...
async function main(): Promise<void> {
  ...
  for (const table of tables) {
    ...
    try {
      ...
      logger.info(
        { table, rowCount: result.rowCount, exportMethod: result.exportMethod, durationMs: result.durationMs },
        `Exported ${table}`
      );
    } catch (error) {
  ...
  if (!options.skipAuth) {
    ...
    try {
      ...
      logger.info(
        { rowCount: authRowCount, durationMs: authDuration },
        "Exported auth.users"
      );
    } catch (error) {
