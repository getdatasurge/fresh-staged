...
const LOG_METHODS: LogMethod[] = ['trace', 'debug', 'info', 'warn', 'error', 'fatal'];
...
function createChildDualLogger(consoleChild: Logger, fileChild: Logger): Logger {
  const proxy = new Proxy(consoleChild, {
    get(target, prop: string | symbol) {
      ...
      if (prop === 'child' && typeof value === 'function') {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        return (bindings: any) => {
          return createChildDualLogger(consoleChild.child(bindings), fileChild.child(bindings));
        };
      }
...
export function logMigrationStart(operation: string, details: Record<string, unknown> = {}): void {
