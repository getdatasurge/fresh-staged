...
export function trackHealthChange(event: Omit<WidgetHealthEvent, 'eventType' | 'timestamp'>): void {
  console.warn('trackHealthChange is deprecated - use useWidgetHealthMetrics hook instead');
}

// Deprecated - use useWidgetHealthMetrics hook instead
export function getHealthDistribution(orgId: string): Record<WidgetHealthStatus, number> {
  console.warn('getHealthDistribution is deprecated - use useWidgetHealthMetrics hook instead');
  return {
    healthy: 0,
    degraded: 0,
    stale: 0,
...
export function getBufferedEvents(orgId: string): WidgetHealthEvent[] {
  console.warn('getBufferedEvents is deprecated - use useWidgetHealthMetrics hook instead');
  return [];
}

// Deprecated - use useWidgetHealthMetrics hook instead
export async function flushHealthMetrics(orgId: string): Promise<void> {
  console.warn('flushHealthMetrics is deprecated - use useWidgetHealthMetrics hook instead');
}

// Deprecated - use useWidgetHealthMetrics hook instead
export function getFailuresByLayer(orgId: string): Record<FailingLayer, number> {
  console.warn('getFailuresByLayer is deprecated - use useWidgetHealthMetrics hook instead');
  return {
    sensor: 0,
    gateway: 0,
    ttn: 0,
...
export function hasCriticalIssues(orgId: string): boolean {
  console.warn('hasCriticalIssues is deprecated - use useWidgetHealthMetrics hook instead');
  return false;
}

// Deprecated - use useWidgetHealthMetrics hook instead
export function resetOrgCounters(orgId: string): void {
  console.warn('resetOrgCounters is deprecated - use useWidgetHealthMetrics hook instead');
}

// Deprecated - use useWidgetHealthMetrics hook instead
export function clearAllCounters(): void {
  console.warn('clearAllCounters is deprecated - use useWidgetHealthMetrics hook instead');
}
