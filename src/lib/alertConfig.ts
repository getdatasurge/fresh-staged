...
import { AlertTriangle, Thermometer, WifiOff, Clock, Battery, type LucideIcon } from 'lucide-react';
...
export const ALERT_TYPE_CONFIG: Record<
  string,
  { icon: LucideIcon; label: string; clearText: string }
> = {
  // Database alert types (snake_case)
  alarm_active: {
    icon: Thermometer,
    label: 'Temperature Alarm',
    clearText: 'Temperature returns to range',
  },
  monitoring_interrupted: {
    icon: WifiOff,
    label: 'Monitoring Interrupted',
    clearText: 'Sensor comes back online',
  },
  missed_manual_entry: {
    icon: Clock,
    label: 'Missed Manual Entry',
    clearText: 'Log a temperature reading',
  },
  ...
  calibration_due: {
    ...
    clearText: 'Calibrate the sensor',
  },
  temp_excursion: {
    icon: Thermometer,
  ...
  suspected_cooling_failure: {
    icon: Thermometer,
    label: 'Suspected Cooling Failure',
    clearText: 'Cooling system restored',
  },

  // Computed alert types (UPPER_CASE - frontend only)
  MANUAL_REQUIRED: {
    icon: Clock,
    label: 'Manual Logging Required',
    clearText: 'Log a temperature reading',
  },
