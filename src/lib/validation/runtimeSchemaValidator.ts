...
export function validatePayloadSchema(
  ...
  const unexpectedFields = payloadKeys.filter((key) => !allSchemaFields.includes(key));
...
export function inferPayloadType(
  ...
  for (const [type, schema] of Object.entries(PAYLOAD_SCHEMAS)) {
    ...
    const score =
      (requiredMatches.length * 2 + optionalMatches.length) /
  ...
  if (isAmbiguous) {
    reasons.push(
      `Ambiguous: ${alternates.map((a) => a.type).join(', ')} also matched with similar confidence`,
    );
  }
