...
import type { SubscriptionPlan, SubscriptionResponse, SubscriptionStatus } from '@/lib/api-types';
...
export const BillingTab = ({ organizationId, canManageBilling }: BillingTabProps) => {
  ...
  const sensorUsage = sensorLimit > 0 ? Math.round((currentSensorCount / sensorLimit) * 100) : 0;
              ...
              {(Object.entries(STRIPE_PLANS) as [PlanKey, (typeof STRIPE_PLANS)[PlanKey]][]).map(
                ([key, plan]) => (
                  <PlanCard
                    ...
                    disabled={
                      currentPlan === key || !!(subscription?.stripeSubscriptionId && !plan.priceId)
                    }
                  />
                ),
              )}
