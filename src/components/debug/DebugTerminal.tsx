...
export function DebugTerminal() {
  ...
  // Extract values from debug context, with defaults for when context is null
  const isDebugEnabled = debugContext?.isDebugEnabled ?? false;
  const isTerminalVisible = debugContext?.isTerminalVisible ?? false;
  const isPaused = debugContext?.isPaused ?? false;
  const logs = debugContext?.logs ?? [];
  const hideTerminal = debugContext?.hideTerminal ?? (() => {});
  const clearLogs = debugContext?.clearLogs ?? (() => {});
  const pauseLogging = debugContext?.pauseLogging ?? (() => {});
  const resumeLogging = debugContext?.resumeLogging ?? (() => {});
  const selectedErrorForExplanation = debugContext?.selectedErrorForExplanation ?? null;
  const showExplanation = debugContext?.showExplanation ?? (() => {});
  const hideExplanation = debugContext?.hideExplanation ?? (() => {});

  // Get user email from Stack Auth
