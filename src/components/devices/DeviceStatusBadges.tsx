...
export function TTNStatusBadge({ provisioningState, ttnDeviceId, className }: TTNStatusBadgeProps) {
  ...
  return (
    <Tooltip>
      ...
      <TooltipContent>
        {isProvisioned ? `Provisioned as ${ttnDeviceId}` : `State: ${provisioningState}`}
      </TooltipContent>
    </Tooltip>
  );
}
  ...
  return (
    <Tooltip>
      <TooltipTrigger asChild>
        <Badge
          variant="outline"
          className={cn('gap-1', isRecent ? 'text-green-600' : 'text-muted-foreground', className)}
        >
          <Clock className="w-3 h-3" />
  ...
  return (
    <Tooltip>
      <TooltipTrigger asChild>
        <Badge
          variant="outline"
          className={cn('gap-1 bg-yellow-500/10 text-yellow-600 border-yellow-500/30', className)}
        >
