...
const StatusBadgeWithTooltip = ({ status }: { status: LoraSensorStatus }) => {
  ...
  return (
    <Tooltip>
      <TooltipTrigger asChild>
        <Badge variant={statusConfig.variant} className={cn('cursor-help', statusConfig.className)}>
          {statusConfig.label}
        </Badge>
      </TooltipTrigger>
    ...
    >
      ...
      >
        <div className="flex items-center gap-1.5">
          ...
          <SelectValue>{currentSiteName || 'Assign Site'}</SelectValue>
        </div>
      </SelectTrigger>
      <SelectContent>
        {sensor.site_id && (
    ...
    >
      ...
      >
        <div className="flex items-center gap-1.5">
          ...
          <SelectValue>{currentUnitName || 'Assign Unit'}</SelectValue>
        </div>
      </SelectTrigger>
      <SelectContent>
        {sensor.unit_id && (
...
export function SensorManager({
  ...
  const isTtnConfiguredNow = Boolean(
    ttnConfig?.isEnabled && ttnConfig?.hasApiKey && ttnConfig?.applicationId,
  );

  // Helper to check if a sensor can be checked NOW
  const canCheckSensorNow = (sensor: LoraSensor) => isTtnConfiguredNow && !!sensor.dev_eui;

  const handleCheckAllTtn = async () => {
    if (!sensors?.length) return;

    // Use current TTN config eligibility, not stored provisioning_state
    const sensorIds = sensors.filter((s) => canCheckSensorNow(s)).map((s) => s.id);
  ...
  const handleDiagnose = async (sensor: LoraSensor) => {
    ...
    try {
      ...
      const checksMap = data.checks.reduce(
        (acc, check) => {
          acc[check.name] = {
            ok: check.passed,
            status: check.passed ? 200 : 404,
            error: check.message,
          };
          return acc;
        },
  ...
  const executeSiteUpdate = async (sensor: LoraSensor, newSiteId: string | null) => {
    ...
    try {
      ...
      toast.success(
        newSiteId ? `Sensor assigned to ${getSiteName(newSiteId)}` : 'Sensor unassigned from site',
      );
    } catch (error) {
  ...
  const executeUnitUpdate = async (sensor: LoraSensor, newUnitId: string | null) => {
    ...
    try {
      ...
      toast.success(
        newUnitId ? `Sensor assigned to ${getUnitName(newUnitId)}` : 'Sensor unassigned from unit',
      );
    } catch (error) {
                              ...
                              <Tooltip>
                                ...
                                <TooltipContent className="max-w-xs">
                                  ...
                                  <p className="text-xs text-muted-foreground mt-1">
                                    This sensor needs AppKey for TTN provisioning. Edit the sensor
                                    to add credentials.
                                  </p>
                                </TooltipContent>
                              </Tooltip>
                            ...
                            <TooltipTrigger asChild>
                              <div className="flex items-center gap-1">
                                ...
                                <Button
                                  ...
                                  onClick={() =>
                                    handleCopy(
                                      sensor.dev_eui.toLowerCase().replace(/[:\-\s]/g, ''),
                                      'DevEUI',
                                    )
                                  }
                                >
                          ...
                          <div className="flex flex-col gap-2">
                            ...
                            <div className="flex flex-col items-start gap-1">
                              <TtnProvisioningStatusBadge
                                state={
                                  (sensor.provisioning_state || 'unknown') as TtnProvisioningState
                                }
                              ...
                              <TtnActions
                                ...
                                isCheckingTtn={
                                  checkingSensorId === sensor.id ||
                                  (checkTtnStatus.isPending && checkingSensorId === null)
                                }
                                ...
                                onDiagnose={
                                  isTtnConfiguredNow ? () => handleDiagnose(sensor) : undefined
                                }
      ...
      <AlertDialog open={!!deleteSensor_} onOpenChange={(open) => !open && setDeleteSensor(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Are you sure you want to archive "{deleteSensor_?.name}"? The sensor will be moved to
              Recently Deleted and can be restored by an admin. All reading history will be
              preserved.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            ...
            >
              {deleteSensor.isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : 'Archive'}
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="space-y-2">
              <p>
                This sensor is currently assigned to{' '}
                <strong>{confirmSiteChange?.currentSiteName}</strong>.
              </p>
              <p>
                {confirmSiteChange?.newSiteId ? (
                  <>
                    Moving it to <strong>{confirmSiteChange?.newSiteName}</strong> may affect data
          ...
          <AlertDialogFooter>
            ...
            >
              {updatingSensorId ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : null}
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="space-y-2">
              <p>
                This sensor is currently monitoring{' '}
                <strong>{confirmUnitChange?.currentUnitName}</strong>.
              </p>
              <p>
                {confirmUnitChange?.newUnitId ? (
                  <>
                    Moving it to <strong>{confirmUnitChange?.newUnitName}</strong> will change what
          ...
          <AlertDialogFooter>
            ...
            >
              {updatingUnitSensorId ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : null}
