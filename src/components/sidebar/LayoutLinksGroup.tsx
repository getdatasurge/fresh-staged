    ...
    <div className="flex flex-col gap-0.5 pl-4">
      ...
      {layout1 ? (
        <Link
          ...
          className={cn(
            linkBaseClass,
            isLayoutActive(layout1.layoutId, false) ? activeClass : inactiveClass,
          )}
        >
          <Layout className="w-3.5 h-3.5" />
          <span className="truncate">{layout1.name}</span>
          {layout1.isUserDefault && (
      ...
      {layout2 && (
        <Link
          ...
          className={cn(
            linkBaseClass,
            isLayoutActive(layout2.layoutId, false) ? activeClass : inactiveClass,
          )}
        >
          <Layout className="w-3.5 h-3.5" />
          <span className="truncate">{layout2.name}</span>
          {layout2.isUserDefault && (
            <Star className="w-3 h-3 text-amber-500 fill-amber-500 ml-auto" />
          )}
        </Link>
      )}
      ...
      {layout3 && (
        <Link
          ...
          className={cn(
            linkBaseClass,
            isLayoutActive(layout3.layoutId, false) ? activeClass : inactiveClass,
          )}
        >
          <Layout className="w-3.5 h-3.5" />
          <span className="truncate">{layout3.name}</span>
          {layout3.isUserDefault && (
            <Star className="w-3 h-3 text-amber-500 fill-amber-500 ml-auto" />
          )}
        </Link>
      )}
