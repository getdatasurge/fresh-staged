...
const BrandedLogo = ({ showText = true, size = 'md' }: BrandedLogoProps) => {
  ...
  if (branding?.logoUrl) {
    return (
      <div className="flex items-center gap-3">
        ...
        />
        {showText && (
          <span className={`${textSizes[size]} font-bold text-foreground`}>{branding.name}</span>
        )}
      </div>
    );
  }
  ...
  return (
    <div className="flex items-center gap-3">
      <div
        className={`${sizeClasses[size]} rounded-lg bg-gradient-to-br from-accent to-primary flex items-center justify-center`}
      >
