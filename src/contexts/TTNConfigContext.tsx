...
export function TTNConfigProvider({ children }: TTNConfigProviderProps) {
  ...
  const isOperationAllowed = useCallback(
    (requiredStates: TTNConfigState[] = ['validated', 'canonical']) => {
  ...
  return <TTNConfigContext.Provider value={value}>{children}</TTNConfigContext.Provider>;
}
