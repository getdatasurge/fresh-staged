...
export function useTTNSetupWizard(organizationId: string | null) {
  ...
  const saveAndConfigureMutation = useMutation(trpc.ttnSettings.saveAndConfigure.mutationOptions());
  ...
  const loadSettings = useCallback(async () => {
    ...
    try {
      ...
      // Use tRPC query data directly
      const result = await ttnSettingsQuery.refetch();
  ...
  const testConnection = useCallback(
    async (sensorId?: string) => {
      ...
      try {
        ...
        const mutationResult = await testConnectionMutation.mutateAsync({
          organizationId,
          deviceId: sensorId,
        });
  ...
  const saveApiKey = useCallback(
    async (apiKey: string) => {
      ...
      try {
        ...
        await saveAndConfigureMutation.mutateAsync({
          organizationId,
