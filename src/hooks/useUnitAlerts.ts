...
export function computeUnitAlerts(
  ...
  for (const unit of units) {
    ...
    const doorContext =
      doorState === 'open' ? ' (door open)' : doorState === 'closed' ? ' (door closed)' : '';
    ...
    if (computed.manualRequired && isManualLoggingEnabled) {
      const hoursOverdue =
        computed.manualOverdueMinutes >= 60 ? Math.floor(computed.manualOverdueMinutes / 60) : 0;
 ...
 * Uses tRPC procedures from backend implementation
 */
 ...
 * Uses tRPC procedures from backend implementation
 */
export function useFetchAlerts(params?: {
  status?: string | string[];
  unitId?: string;
  siteId?: string;
  page?: number;
