...
export function useTTNDeprovisionJobs(
	orgId: string | null,
	statusFilter?: string[],
) {
	const user = useUser()
...
export function useScanTTNOrphans() {
	...
	return useMutation({
		mutationFn: async (organizationId: string) => {
			...
			throw new Error(
				'TTN deprovision scan unavailable during Supabase removal',
			)
		},
	})
}
...
export function useEnqueueOrphanCleanup() {
	...
	return useMutation({
		mutationFn: async (variables: {
			...
			if (!user) throw new Error('Not authenticated')
			throw new Error(
				'TTN deprovision queue unavailable during Supabase removal',
			)
		},
...
export function useRetryDeprovisionJob(orgId: string | null) {
	...
	return useMutation({
		mutationFn: async (jobId: string): Promise<void> => {
			if (!user) throw new Error('Not authenticated')
			throw new Error(
				'TTN deprovision retry unavailable during Supabase removal',
			)
		},
		onSuccess: async () => {
