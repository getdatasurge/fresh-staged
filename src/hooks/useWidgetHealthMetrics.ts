...
export function useWidgetHealthMetrics() {
   ...
   */
  const trackHealthChange = async (event: Omit<WidgetHealthEvent, 'eventType' | 'timestamp'>) => {
    try {
   ...
   */
  const getFailuresByLayer = async (orgId: string): Promise<Record<FailingLayer, number>> => {
    try {
   ...
   */
  const getBufferedEvents = async (orgId: string): Promise<WidgetHealthEvent[]> => {
    try {
