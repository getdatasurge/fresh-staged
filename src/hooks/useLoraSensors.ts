 ...
 */
export function useLoraSensorByDevEui(
	devEui: string | null,
	orgId?: string | null,
) {
	const { orgId: scopeOrgId, isReady } = useOrgScope()
 ...
 */
export function useLoraSensorsByUnit(
	unitId: string | null,
	orgId?: string | null,
) {
	const { orgId: scopeOrgId, isReady } = useOrgScope()
...
export function useLinkSensorToUnit() {
	...
	return useMutation({
		...
		onSuccess: (data, variables) => {
			...
			toast.success(
				data.unitId ? 'Sensor linked to unit' : 'Sensor unlinked from unit',
			)
		},
		onError: (error: Error) => {
...
const getProvisionErrorMessage = (error: string): string => {
	...
	if (
		...
		error.includes('TTN not configured')
	) {
	...
	if (
		error.includes('end_device_euis_taken') ||
		error.includes('already registered')
	) {
		// Try to extract device_id and application from the error message
		const deviceIdMatch = error.match(
			/device[_\s]?id[:\s]+['"]?([a-z0-9-]+)['"]?/i,
		)
		const applicationMatch = error.match(
			/application[_\s]?(?:id)?[:\s]+['"]?([a-z0-9-]+)['"]?/i,
		)
		...
		if (deviceIdMatch || applicationMatch) {
			...
			details +=
				". Use 'Check Status' to sync, or delete the device in TTN Console first."
		} else if (devEuiMatch) {
