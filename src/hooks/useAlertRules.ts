...
export function computeOfflineSeverity(
	...
	if (missedCheckins >= rules.offline_critical_missed_checkins)
		return 'critical'
...
function mapBackendToFrontend(rule: any): AlertRulesRow {
	...
	return {
		...
		excursion_confirm_minutes_door_closed:
			rule.excursionConfirmMinutesDoorClosed,
...
function mapFrontendToBackend(rules: Partial<AlertRulesRow>): any {
	return {
		...
		excursionConfirmMinutesDoorClosed:
			rules.excursion_confirm_minutes_door_closed,
		...
		manualLogMissedCheckinsThreshold:
			rules.manual_log_missed_checkins_threshold,
	}
}
