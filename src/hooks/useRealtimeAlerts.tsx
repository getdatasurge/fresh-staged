...
export function useRealtimeAlerts(organizationId: string | undefined) {
  ...
  useEffect(() => {
    ...
    function handleAlertTriggered(data: AlertNotification) {
      ...
      toast.custom(
        (t) => <AlertToast type="triggered" message={data.message} severity={data.severity} />,
        {
      ...
      queryClient.setQueryData(qk.unit(data.unitId).status(), (old: any) =>
        old ? { ...old, status: 'excursion' } : old,
      );
    }

    function handleAlertResolved(data: { alertId: string; unitId: string; resolvedAt: string }) {
      toast.custom(
        (t) => <AlertToast type="resolved" message="Temperature returned to acceptable range" />,
        { duration: 5000 },
      );
      ...
      // Update unit status
      queryClient.setQueryData(qk.unit(data.unitId).status(), (old: any) =>
        old ? { ...old, status: 'restoring' } : old,
      );
    }

    function handleAlertEscalated(data: {
      alertId: string;
      unitId: string;
      ...
      queryClient.setQueryData(qk.unit(data.unitId).status(), (old: any) =>
        old ? { ...old, status: 'alarm_active' } : old,
      );
    }
