...
export function useEffectiveIdentity(): EffectiveIdentity {
  ...
  const { isSuperAdmin, rolesLoaded, impersonation, isSupportModeActive } = useSuperAdmin();
  ...
  const {
    ...
    queryFn: async () => {
      ...
      try {
        ...
        const trpcClient = createTRPCClientInstance(async () => authJson.accessToken);
  ...
  const isImpersonating = Boolean(isSuperAdmin && isSupportModeActive && hasContextImpersonation);
  ...
  const effectiveOrgName = isImpersonating ? (impersonation.impersonatedOrgName ?? null) : null;
