...
export async function softDeleteUnit(
  ...
  try {
    ...
    // Also soft delete associated devices if cascade
    if (cascade) {
      const now = new Date().toISOString();
      await supabase
        .from("devices")
...
export async function softDeleteArea(
  ...
  try {
    ...
    // Check for active units if not cascading
    if (!cascade) {
    ...
    // TODO: Backend should handle cascade deletion
    // For now, cascade delete units manually before deleting area
    if (cascade) {
    ...
    // Delete area via new API
    await areasApi.deleteArea(
      info.organizationId,
      info.siteId!,
      areaId,
      accessToken
    ...
    await logEvent({
      ...
      event_data: {
        ...
        cascade_deleted_units: cascade ? 1 : 0, // Approximate
      },
    });
...
export async function softDeleteSite(
  ...
  try {
    ...
    // Check for active areas if not cascading
    if (!cascade) {
    ...
    // TODO: Backend should handle cascade deletion
    // For now, cascade delete areas and their units manually before deleting site
    if (cascade) {
      ...
      if (areas) {
        for (const area of areas) {
          // Delete area with cascade to handle its units
          await softDeleteArea(area.id, userId, true, accessToken);
        }
      }
    }

    // Delete site via new API
    await sitesApi.deleteSite(info.organizationId, siteId, accessToken);
    ...
    await logEvent({
      ...
      event_data: {
        entity_type: "site",
        entity_name: info.name,
        cascade_deleted_areas: cascade ? 1 : 0, // Approximate
      },
    });
...
/**
 * Hook to access soft delete functions with Stack Auth context
 */
export function useSoftDelete() {
  const user = useUser();

  return {
    softDeleteUnit: async (unitId: string, userId: string, cascade?: boolean) => {
      if (!user) throw new Error('Not authenticated');
      const { accessToken } = await user.getAuthJson();
      return softDeleteUnit(unitId, userId, cascade, accessToken);
    },
    softDeleteArea: async (areaId: string, userId: string, cascade?: boolean) => {
      if (!user) throw new Error('Not authenticated');
      const { accessToken } = await user.getAuthJson();
      return softDeleteArea(areaId, userId, cascade, accessToken);
    },
    softDeleteSite: async (siteId: string, userId: string, cascade?: boolean) => {
      if (!user) throw new Error('Not authenticated');
      const { accessToken } = await user.getAuthJson();
      return softDeleteSite(siteId, userId, cascade, accessToken);
    },
    softDeleteDevice,
    softDeleteSensor,
    restoreUnit,
