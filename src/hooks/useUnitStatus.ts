...
/**
 * Fetch unit status from nav tree by unitId
 *
 * Why this approach:
 * - Nav tree already loads all units with status field
 * - No dedicated /api/orgs/:orgId/units/:unitId endpoint (units are hierarchical)
 * - Creating a unit-by-id endpoint just for status is unnecessary overhead
 *
 * Returns unit status data derived from nav tree (no separate API call)
 */
export function useFetchUnitStatus(unitId: string | null) {
  const { orgId, isReady } = useOrgScope();
  const navTree = useNavTree(orgId);

  return useMemo(() => {
    if (!isReady || !unitId || navTree.isLoading) {
      return {
        data: null,
        isLoading: navTree.isLoading,
        error: navTree.error,
      };
    }
