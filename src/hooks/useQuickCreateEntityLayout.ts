...
export function useQuickCreateEntityLayout() {
  ...
  return useMutation({
    mutationFn: async (params: CreateLayoutParams): Promise<CreatedLayout> => {
      ...
      // Dashboard layouts should be created via /api/layouts endpoint
      // Backend handles user_id from Stack Auth token
      console.warn('[useQuickCreateEntityLayout] TODO: migrate to new backend');
