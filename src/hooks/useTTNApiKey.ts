      ...
      if (data?.valid || data?.ok) {
        ...
        setValidated({
          ...
          permissions: data.permissions
            ? {
                applications_read:
                  data.permissions.rights?.includes('RIGHT_APPLICATION_INFO') ?? false,
                applications_write:
                  data.permissions.rights?.includes('RIGHT_APPLICATION_SETTINGS_BASIC') ?? false,
                devices_read:
                  data.permissions.rights?.includes('RIGHT_APPLICATION_DEVICES_READ') ?? false,
                devices_write:
                  data.permissions.rights?.includes('RIGHT_APPLICATION_DEVICES_WRITE') ?? false,
                gateways_read: false,
