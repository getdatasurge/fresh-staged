...
const RecentlyDeleted = () => {
  ...
  const filteredItems =
    activeTab === 'all' ? items : items.filter((item) => item.entityType === activeTab);
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          <div>
            ...
            <CardDescription>
              View and restore deleted sites, areas, units, and devices. Items can be permanently
              deleted if needed.
            </CardDescription>
          </div>
          ...
          <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as typeof activeTab)}>
            <TabsList className="mb-4">
              ...
              <TabsTrigger value="site">
                Sites ({items.filter((i) => i.entityType === 'site').length})
              </TabsTrigger>
              <TabsTrigger value="area">
                Areas ({items.filter((i) => i.entityType === 'area').length})
              </TabsTrigger>
              <TabsTrigger value="unit">
                Units ({items.filter((i) => i.entityType === 'unit').length})
              </TabsTrigger>
              <TabsTrigger value="device">
                Devices ({items.filter((i) => i.entityType === 'device').length})
              </TabsTrigger>
              <TabsTrigger value="sensor">
                Sensors ({items.filter((i) => i.entityType === 'sensor').length})
              </TabsTrigger>
            </TabsList>
              ...
              {filteredItems.length === 0 ? (
                <div className="text-center py-12 text-muted-foreground">
                  ...
                  <p className="text-sm mt-2">
                    Recently deleted listings are being migrated to tRPC.
                  </p>
                </div>
              ) : (
                <Table>
