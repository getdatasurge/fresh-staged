...
export function EntityDashboard({
  ...
  const validation = useLayoutValidation(state.activeLayout.config, entityType, {
    hasSensor: !!sensor,
  ...
  const handleLayoutChange = useCallback(
    (layout: WidgetPosition[]) => actions.updatePositions(layout),
    [actions],
  );
  const handleRestoreWidget = useCallback(
    (widgetId: string) => actions.toggleWidgetVisibility(widgetId),
    [actions],
  );
  const handleRestoreAllWidgets = useCallback(() => {
    (state.activeLayout.config.hiddenWidgets || []).forEach((id) =>
      actions.toggleWidgetVisibility(id),
    );
  }, [actions, state.activeLayout.config.hiddenWidgets]);

  const handleAddWidget = useCallback(
    (widgetId: string) => {
      const maxY = state.activeLayout.config.widgets.reduce(
      ...
      if (state.activeLayout.config.hiddenWidgets?.includes(widgetId))
  ...
  return (
    <DashboardErrorBoundary entityType={entityType} entityId={entityId} userId={userId}>
        ...
        <AlertDialog open={unsavedGuard.showPrompt}>
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                You have unsaved changes to this layout. Would you like to save them before
                navigating away?
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
              <AlertDialogCancel onClick={unsavedGuard.onCancel}>Cancel</AlertDialogCancel>
              <Button variant="outline" onClick={unsavedGuard.onDiscard}>
                Discard
              </Button>
              <AlertDialogAction onClick={unsavedGuard.onSave} disabled={unsavedGuard.isSaving}>
        ...
        <Dialog open={createLayoutPromptOpen} onOpenChange={setCreateLayoutPromptOpen}>
          <DialogContent className="sm:max-w-md">
            <DialogHeader>
              ...
              <DialogDescription>
                Widgets can only be added to custom layouts. Create a layout to start customizing
                your dashboard.
              </DialogDescription>
            </DialogHeader>
            <DialogFooter className="flex-col sm:flex-row gap-2">
              ...
              <Button onClick={handleCreateFirstLayout}>Create Layout</Button>
            </DialogFooter>
          </DialogContent>
        </Dialog>
      </div>
