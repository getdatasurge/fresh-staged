...
export function WidgetEmptyState({
  ...
  if (state.status === 'loading') {
    return (
      <div className={cn('flex flex-col items-center justify-center h-full py-4', className)}>
        <div className={cn('rounded-full p-2', styles.iconBg)}>
          <Loader2 className={cn('w-5 h-5 animate-spin', styles.iconColor)} />
        </div>
        <p className={cn('text-xs mt-2', styles.textColor)}>{state.message || 'Loading...'}</p>
      </div>
    );
  }
  ...
  return (
    ...
    >
      ...
      >
        <Icon className={cn(styles.iconColor, compact ? 'w-4 h-4' : 'w-5 h-5')} />
      </div>

      {/* State Message */}
      <p className={cn('font-medium', styles.textColor, compact ? 'text-xs' : 'text-sm')}>
        {state.message}
      </p>

      {/* Root Cause */}
      {state.rootCause && !compact && (
        <p className="text-xs text-muted-foreground max-w-[200px]">{state.rootCause}</p>
      )}
