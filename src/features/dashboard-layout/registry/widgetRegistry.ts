...
export const WIDGET_REGISTRY: Record<string, WidgetDefinition> = {
  ...
  temperature_chart: {
    ...
    description:
      'Interactive chart showing temperature readings over time with zoom and comparison features.',
    mandatory: true,
    minW: 4,
    minH: 4,
    maxW: 12,
  ...
  temperature_statistics: {
    ...
    description:
      'Min, max, and average temperature for the selected period with visual indicators.',
    mandatory: false,
    minW: 3,
    minH: 2,
    maxW: 8,
  ...
  unit_compliance_score: {
    ...
    description:
      'Overall compliance percentage with breakdown by readings, logs, and alert response.',
    mandatory: false,
    minW: 3,
    minH: 2,
    maxW: 8,
...
export function getWidgetsForEntity(entityType: EntityType): WidgetDefinition[] {
  return getAllWidgets().filter((w) => !w.entityTypes || w.entityTypes.includes(entityType));
}
...
export function validateMandatoryWidgets(
  ...
  const missingWidgets = mandatory.filter((w) => !visibleWidgetIds.includes(w.id)).map((w) => w.id);
