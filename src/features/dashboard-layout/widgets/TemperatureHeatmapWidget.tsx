...
export function TemperatureHeatmapWidget({ site, organizationId }: WidgetProps) {
  ...
  const heatmapData: UnitHeatmapData[] = useMemo(() => {
    ...
    return units.map((unit) => {
      const lastReadingTime = unit.lastReadingAt ? new Date(unit.lastReadingAt).getTime() : 0;
      const isOffline = !unit.lastReadingAt || now - lastReadingTime > offlineThresholdMs;
                ...
                <Tooltip key={unit.id}>
                  ...
                  <TooltipContent>
                    <div className="text-xs">
                      ...
                      {unit.lastReadingAt && (
                        <p>Last reading: {format(new Date(unit.lastReadingAt), 'MMM d, h:mm a')}</p>
                      )}
                    </div>
                  </TooltipContent>
                </Tooltip>
