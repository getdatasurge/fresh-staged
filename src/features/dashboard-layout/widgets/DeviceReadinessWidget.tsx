...
export function DeviceReadinessWidget({
  ...
  const effectiveBatteryLevel = hasLoraData
    ...
    : (primarySensor?.battery_level ?? device?.battery_level);
  const effectiveSignalStrength = hasLoraData
    ? loraSensor.signal_strength
    : (primarySensor?.signal_strength ?? device?.signal_strength);
  const effectiveLastSeen = hasLoraData
    ? loraSensor.last_seen_at
    : derivedStatus?.lastSeenAt || device?.last_seen_at;
