...
export function DeviceStatusWidget({ derivedStatus, unitType }: DeviceStatusWidgetProps) {
  return (
    <div className="h-full p-4">
      <p className="text-sm font-medium text-muted-foreground mb-3">Device Status</p>
      <div className="flex items-center gap-3">
        <div
          className={`w-10 h-10 rounded-lg flex items-center justify-center ${derivedStatus.statusBgColor}`}
        >
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-1">{unitType.replace(/_/g, ' ')}</p>
        </div>
      </div>
    </div>
  );
