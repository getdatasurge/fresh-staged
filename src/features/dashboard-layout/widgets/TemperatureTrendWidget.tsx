...
export function TemperatureTrendWidget({ readings = [], sensor, loraSensors }: WidgetProps) {
  ...
  const trend = useMemo(() => {
    ...
    const recentReadings = sorted.filter((r) => new Date(r.recorded_at).getTime() > hourAgo);
  ...
  const widgetState = useMemo(() => {
    ...
    if (!readings || readings.length === 0) {
      return createEmptyState('Waiting for sensor readings...', 'Data collection in progress');
    }
      ...
      <div className="flex-1 min-h-0 overflow-y-auto px-4 pb-4">
        <div className="flex items-center justify-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">{Math.abs(trend.rate).toFixed(1)}°/hour</p>
          </div>
        </div>
