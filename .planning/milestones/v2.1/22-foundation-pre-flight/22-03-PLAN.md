...
must_haves:
  truths:
    ...
    - 'Script provides rollback guidance on critical failures with specific commands'
  ...
  key_links:
    ...
    - from: 'error_handler function'
      to: 'handle_recovery function'
      via: 'direct call in error_handler before exit'
      pattern: "handle_recovery.*\\$category"
---
...
**3. Update error_handler to use save_error_state and handle_recovery:**

Find the existing error_handler function and update it to:

1. Save error state after diagnostics output
2. Call handle_recovery before exit to prompt user

Add these lines after the diagnostics output block, before the exit:
...
This ensures handle_recovery is automatically called from error_handler, so recovery prompts appear automatically on failures.
</action>
<verify>
...
- PASS: validate\_\* functions working
...
- ERROR-04: Critical failures trigger rollback guidance with specific commands (docker compose logs, journalctl -xe, ./scripts/rollback.sh)
- ERROR-05: Recoverable failures prompt user with fix suggestions and retry option
- error_handler automatically calls handle_recovery for interactive prompts
