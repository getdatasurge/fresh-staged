...
Test credential sanitization in error output:

```bash
bash -c 'source scripts/lib/preflight-lib.sh; export MY_PASSWORD=secret123; somecommand --password=secret123' 2>&1 | grep -v "secret123" && echo "PASS: Credentials redacted"
```

Error output should show `[REDACTED]` instead of the actual secret value.
</verify>
<done>
