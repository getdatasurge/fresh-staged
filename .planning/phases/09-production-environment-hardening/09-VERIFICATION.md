...
| `backend/.dockerignore`                       | Backend build context secret exclusion                | ✓ VERIFIED | 102 lines, 11+ secret patterns (*.key, *.pem, .env, secrets/, etc.)           |
...
| backend/.dockerignore         | Secret exclusion         | File pattern matching                | ✓ WIRED    | Patterns exclude *.key, *.pem, .env, secrets/, credentials.json |
...
- `backend/.dockerignore`: 11+ secret exclusion patterns (*.key, *.pem, *.p12, *.pfx, .env, secrets/, credentials.json, service-account*.json, id_rsa, config/production.*)
