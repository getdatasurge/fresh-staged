...
must_haves:
  truths:
    - "User can identify common deployment failures from error messages"
    - "User can find the correct fix for each failure type"
    - "User can diagnose issues using provided commands"
    - "User can recover from checkpoint failures"
  artifacts:
