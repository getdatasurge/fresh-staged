...
must_haves:
  truths:
    - 'Worker health endpoint validation is available for deployment verification'
    - 'Monitoring stack validation (Prometheus/Grafana) is available'
    - 'Combined service validation function checks all endpoints in one call'
    - 'Dashboard can be verified with 3 consecutive health passes'
  artifacts:
...
**VERIFY-06 scope clarification:** This function is specifically for the dashboard health check (VERIFY-03 + VERIFY-06). Other endpoints in verify_all_services() use standard single-check with built-in retries. The 3-consecutive-pass requirement applies ONLY to the dashboard check, matching the Phase 34 pattern where deploy-lib.sh applies 3-pass only to the main health endpoint.
...
<done>verify_consecutive_health() function exists with configurable consecutive pass requirement, scoped for dashboard use</done>
</task>
...
3. verify_consecutive_health() function implements 3-consecutive-pass pattern (for dashboard only)
