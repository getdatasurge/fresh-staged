...
must_haves:
  ...
  key_links:
    ...
    - from: "scripts/deploy-orchestrated.sh"
      ...
      via: "function call integration"
      pattern: "deploy\\.sh|run_deploy"
...
Purpose: Enable checkpoint-based deployment with resume capability from any failure point, integrating with existing deploy.sh without code duplication

Output:
- scripts/lib/deploy-lib.sh - Deployment state management extending preflight-lib.sh checkpoint system
- scripts/deploy-orchestrated.sh - Main orchestration script that calls deploy.sh phases
</objective>
...
Use consistent style with preflight-lib.sh (colors, step/success/error helpers inherited via source).
  ...
  <verify>
Run `bash scripts/lib/deploy-lib.sh` directly - self-tests should pass.
Verify `source scripts/lib/deploy-lib.sh` works without errors.
Check that STATE_DIR/.deployment-state is created when set_deployment_state is called.
  </verify>
  <done>
deploy-lib.sh exists with deployment_checkpoint(), get_resume_point(), set_deployment_state(), show_deployment_status() functions. Self-tests pass. Library sources without error.
  </done>
</task>
...
Define phase functions that wrap existing functionality:

1. `phase_preflight()` - Run system validation: Call `run_preflight_checks "$DOMAIN"` from preflight-lib.sh
...
4. `phase_pull_images()` - Pull Docker images: Run `docker compose -f docker-compose.yml -f compose.production.yaml pull`

5. `phase_build_backend()` - Build backend: Run `docker compose -f docker-compose.yml -f compose.production.yaml build backend`

6. `phase_database_start()` - Start database services: Run `docker compose -f docker-compose.yml -f compose.production.yaml up -d postgres redis`

7. `phase_database_migrate()` - Run migrations: Wait for postgres ready (pg_isready loop), then run `docker compose run --rm backend pnpm db:migrate:prod`

8. `phase_services_start()` - Start all services: Run `docker compose -f docker-compose.yml -f compose.production.yaml up -d`

9. `phase_health_wait()` - Wait for healthy: Placeholder that calls health check (enhanced in Plan 02). For now, simple curl loop to /health endpoint.

10. `phase_cleanup()` - Cleanup: Run `docker image prune -f --filter "dangling=true"`
...
IMPORTANT: Do NOT duplicate logic from deploy.sh. The phase functions should be thin wrappers that call docker compose commands directly. The existing deploy.sh remains for backwards compatibility but this script is the new recommended entry point.
...
`./scripts/deploy-orchestrated.sh --status` shows deployment status (should show "not started" on fresh system).
  ...
  <done>
deploy-orchestrated.sh exists with --resume, --fresh, --status flags. Sources deploy-lib.sh. Defines phase_* functions for each deployment step. Main loop uses deployment_checkpoint() for each phase. Script is executable.
  </done>
</task>
...
2. `bash scripts/lib/deploy-lib.sh` - self-tests pass
...
- Checkpoint system allows resume from any failure point
- Docker Compose commands use production overlay: `-f docker-compose.yml -f compose.production.yaml`
- No logic duplication from existing deploy.sh (integration, not rewrite)
</success_criteria>
