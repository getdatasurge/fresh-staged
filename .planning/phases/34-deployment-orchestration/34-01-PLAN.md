...
<done>
deploy-orchestrated.sh exists with --resume, --fresh, --status flags. Sources deploy-lib.sh. Defines phase\_\* functions that use same deployment sequence as deploy.sh. Main loop uses deployment_checkpoint() for each phase. Script is executable.
</done>
</task>
