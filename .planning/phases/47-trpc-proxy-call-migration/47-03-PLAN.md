...
1. `grep -n 'trpcClient\.' src/pages/Inspector.tsx` -- should show 5 trpcClient calls
2. `grep -n 'trpcClient\.' src/pages/PilotSetup.tsx` -- should show 2 trpcClient calls
3. `grep -n 'trpcClient\.' src/features/dashboard-layout/widgets/SiteAlertsSummaryWidget.tsx` -- should show 1 trpcClient call
4. `grep -n 'trpcClient\.' src/features/dashboard-layout/widgets/AlertHistoryWidget.tsx` -- should show 1 trpcClient call
5. `grep -n 'mutationOptions' src/pages/Inspector.tsx src/pages/PilotSetup.tsx` -- should show these still use `trpc.` (proxy pattern, correct)
  </verify>
...
4. **If any remaining proxy misuse found in step 1:** Fix those files using the same pattern (add useTRPCClient import, get trpcClient, replace trpc.X.Y.mutate/query with trpcClient.X.Y.mutate/query). Then re-run verification.
  ...
  <verify>
1. `grep -rn 'trpc\.[a-zA-Z]*\.[a-zA-Z]*\.mutate\b\|trpc\.[a-zA-Z]*\.[a-zA-Z]*\.query\b' src/ --include='*.ts' --include='*.tsx'` returns ZERO matches
2. `pnpm run build` exits with code 0
  </verify>
