...
<task type="auto">
  <name>Task 2: Add METER_REPORTING queue and job type definitions</name>
  <files>
    backend/src/jobs/index.ts
    backend/src/services/queue.service.ts
  </files>
  <action>
1. Update `backend/src/jobs/index.ts` to add meter reporting types:

```typescript
// Meter reporting job data (Phase 18)
export interface MeterReportJobData extends BaseJobData {
  /** Stripe meter event name (e.g., 'active_sensors') */
  eventName: string;
  /** Value to report to the meter */
  value: number;
}
...
export const QueueNames = {
  ...
  METER_REPORTING: 'meter-reporting',  // Add this
} as const;
...
export const JobNames = {
  ...
  METER_REPORT: 'meter:report',  // Add this
  SENSOR_COUNT_SCHEDULER: 'meter:sensor-count-scheduler',  // Repeatable scheduler job
} as const;

// Add type helper:
export type MeterReportJob = Job<MeterReportJobData>;

// Add job options for meter reporting:
export const meterReportJobOptions: JobsOptions = {
  attempts: 3,
  backoff: {
    type: 'exponential',
    delay: 5000, // 5s initial (Stripe rate limits)
  },
...
2. Update `backend/src/services/queue.service.ts` in the `initialize()` method to register the METER_REPORTING queue:

```typescript
// In initialize() after other registerQueue calls:
this.registerQueue(QueueNames.METER_REPORTING);
```
  </action>
  <verify>
Run `cd backend && pnpm tsc --noEmit` passes.
Grep for METER_REPORTING in queue.service.ts.
Grep for MeterReportJobData in jobs/index.ts.
  </verify>
  <done>
Queue and job types updated with:
- MeterReportJobData interface with eventName and value
- METER_REPORTING queue name constant
- METER_REPORT and SENSOR_COUNT_SCHEDULER job name constants
- MeterReportJob type helper
- meterReportJobOptions with Stripe-appropriate backoff
- Queue registered in QueueService.initialize()
  </done>
</task>
...
- [ ] `backend/src/jobs/index.ts` has METER_REPORTING queue and MeterReportJobData
- [ ] `backend/src/services/queue.service.ts` registers METER_REPORTING queue
...
3. METER_REPORTING queue registered in QueueService
