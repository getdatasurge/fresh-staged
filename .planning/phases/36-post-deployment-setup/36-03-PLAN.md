...
2. Improve success message with sample data details
...
Update the success message at the end to be more informative:
```bash
...
echo "  Demo Alert:        1 active temperature alert"
echo ""
...
```
  ...
  <verify>
grep -q "Demo Organization" scripts/seed-demo-data.sh && grep -q "pg_isready" scripts/seed-demo-data.sh && echo "Script enhanced"
  </verify>
  <done>seed-demo-data.sh has database readiness check and detailed output</done>
</task>
...
5. Both scripts have proper error handling (set -e)
</verification>
...
4. Credential summary outputs to terminal only (via /dev/tty from post-deploy-lib.sh)
5. Next steps guide is displayed at the end
</success_criteria>
