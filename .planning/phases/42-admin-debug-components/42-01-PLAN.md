...
- Remove the platformRolesCount state and its display section
  ...
  <verify>
    grep -l "supabase-placeholder" src/contexts/SuperAdminContext.tsx src/components/debug/RBACDebugPanel.tsx | wc -l
    # Should return 0
    npx tsc --noEmit
  </verify>
...
- Replace with tRPC useQuery:
  ```typescript
  const trpc = useTRPC()
  const { data: unitsData, isLoading: unitsLoading, refetch: refetchUnits } = useQuery(
...
- Show a banner explaining simulator edge function is unavailable
  ...
  <verify>
    grep "supabase-placeholder" src/components/admin/SensorSimulatorPanel.tsx
...
- The banner still shows useful debug info: session, IDs, counts passed as props, realtime status
  ...
  <verify>
    grep "supabase-placeholder" src/components/debug/UnitDebugBanner.tsx
    # Should return nothing
    npx tsc --noEmit
  </verify>
