...
```typescript
import { QueueNames, defaultJobOptions, smsJobOptions, type BaseJobData, type SmsNotificationJobData } from '../jobs/index.js';
```
...
```typescript
import { QueueNames, JobNames, defaultJobOptions, smsJobOptions, type BaseJobData, type SmsNotificationJobData } from '../jobs/index.js';
```
...
4. Use JobNames.SMS_SEND constant instead of string literal in addSmsJob.
  ...
  <verify>
1. `npx tsc --noEmit` passes
2. QueueService has addSmsJob method
3. addSmsJob uses smsJobOptions with 5 attempts
  </verify>
  <done>
