...
```typescript
async function getAlertRecipients(organizationId: string): Promise<Array<{ userId: string; phoneNumber: string }>> {
...
async function queueAlertSms(
  ...
  for (const recipient of recipients) {
    ...
    console.log(`[Alert SMS] Queued SMS for user ${recipient.userId} to ${recipient.phoneNumber.slice(0, 5)}***`);
  }
}
```
...
if (escalatedAlert) {
  ...
  queueAlertSms(organizationId, escalatedAlert.id, alertMessage, 'alarm_active')
    .catch((err) => {
...
describe('SMS Notification Processor', () => {
  ...
  describe('processSmsNotification', () => {
    const createMockJob = (data: Partial<SmsNotificationJobData> = {}): Job<SmsNotificationJobData> => {
