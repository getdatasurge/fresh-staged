...
export const unitTypeEnum = pgEnum('unit_type', ['freezer', 'refrigerator', 'warmer', 'ambient']);
...
export const sensorReadings = pgTable(
  ...
  (table) => [
    ...
    index('sensor_readings_device_time_idx').on(table.device_id, table.recorded_at.desc()),
    ...
    // Unique constraint
    uniqueIndex('sensor_readings_unique_idx').on(table.device_id, table.recorded_at),
  ],
);
```
