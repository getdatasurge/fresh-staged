...
2. computeTableChecksum(pool, tableName):
   ...
   async function computeTableChecksum(pool: Pool, tableName: string): Promise<string | null> {
     try {
       ...
       logger.warn({ tableName, err }, 'Checksum computation failed, falling back to row count only');
       return null;
     }
   }
   ```
         ...
         if (!opts.skipChecksum) {
           ...
           if (requiresUserMapping(table)) {
             ...
             sourceChecksum = await computeChecksumExcludingColumns(
               supabasePool, table, excludeCols
             );
             targetChecksum = await computeChecksumExcludingColumns(
               newDbPool, table, excludeCols
             );
           } else {
