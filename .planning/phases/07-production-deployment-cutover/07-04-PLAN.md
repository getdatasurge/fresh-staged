...
if [ -f "$PROJECT_ROOT/.env.production" ]; then
    ...
    echo "  ⚠ .env.production not found (optional if using compose defaults)"
fi
...
for port in "${PORTS_TO_CHECK[@]}"; do
    if ! ss -tuln | grep -q ":${port} " 2>/dev/null; then
        ...
        if [ -n "$CONTAINER" ]; then
            echo "  ⚠ Port $port in use by container: $CONTAINER"
        else
...
echo "✓ Backend and Caddy stopped"
echo ""
...
if docker compose -f "$PROJECT_ROOT/docker-compose.yml" \
    ...
    echo "⚠ Database not accessible. Manual export may be needed later."
...
if [ -n "$SLACK_WEBHOOK_URL" ]; then
    curl -s -X POST "$SLACK_WEBHOOK_URL" \
         ...
         -d '{"text":"🔴 FreshTrack Pro ROLLBACK COMPLETE - System reverted to Supabase"}' \
         > /dev/null
    echo "✓ Slack notification sent"
else
    echo "⚠ SLACK_WEBHOOK_URL not set - manual notification needed"
fi
