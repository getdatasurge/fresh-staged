...
describe('Organizations tRPC Router', () => {
  ...
  beforeEach(async () => {
    ...
    const statsService = await import('../../src/services/organization-stats.service.js');
    ...
    mockGetOrganizationStatsService = statsService.getOrganizationStatsService as any;
  });
  ...
  describe('get', () => {
    it('should return organization data', async () => {
      ...
      const result = await caller.get({ organizationId: '123e4567-e89b-42d3-a456-426614174000' });

      expect(result).toEqual(mockOrg);
      expect(mockGetOrganization).toHaveBeenCalledWith('123e4567-e89b-42d3-a456-426614174000');
    });
  ...
  describe('update', () => {
    it('should update organization when user is owner', async () => {
      ...
      expect(mockUpdateOrganization).toHaveBeenCalledWith('123e4567-e89b-42d3-a456-426614174000', {
        name: 'Updated Org',
        timezone: 'America/Los_Angeles',
        complianceMode: 'haccp',
        logoUrl: 'https://example.com/logo.png',
  ...
  describe('listMembers', () => {
    it('should return member list', async () => {
      ...
      const result = await caller.listMembers({ organizationId: '123e4567-e89b-42d3-a456-426614174000' });

      expect(result).toEqual(mockMembers);
      expect(mockListMembers).toHaveBeenCalledWith('123e4567-e89b-42d3-a456-426614174000');
    });
  ...
  describe('stats', () => {
    it('should return organization stats', async () => {
      ...
      const result = await caller.stats({ organizationId: '123e4567-e89b-42d3-a456-426614174000' });

      expect(result).toEqual(mockStats);
      expect(mockStatsService.getOrganizationStats).toHaveBeenCalledWith('123e4567-e89b-42d3-a456-426614174000');
    });
    ...
    it('should throw INTERNAL_SERVER_ERROR when stats service not initialized', async () => {
      ...
      await expect(
        caller.stats({ organizationId: '123e4567-e89b-42d3-a456-426614174000' })
      ).rejects.toThrow(TRPCError);
