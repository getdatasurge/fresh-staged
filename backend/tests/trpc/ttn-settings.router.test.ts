...
describe('TTN Settings tRPC Router', () => {
	...
	beforeEach(async () => {
		...
		const ttnSettingsService =
			await import('../../src/services/ttn-settings.service.js')
	...
	describe('update', () => {
		...
		it('should update settings when user is admin', async () => {
			...
			expect(mockUpdateTTNSettings).toHaveBeenCalledWith(
				orgId,
				updateInput.data,
			)
		})
	...
	describe('test', () => {
		it('should throw BAD_REQUEST when TTN not configured', async () => {
			...
			await expect(caller.test({ organizationId: orgId })).rejects.toThrow(
				TRPCError,
			)
