...
describe('Sites tRPC Router', () => {
  ...
  // Sample site data for get, create, update, delete tests
  const baseMockSite = {
    id: siteId,
  ...
  // Sample site data with stats for list tests
  const siteWithStats = {
    ...baseMockSite,
    areasCount: 2,
    unitsCount: 10,
  ...
  describe('list', () => {
    it('should list sites for organization', async () => {
      ...
      mockListSitesWithStats.mockResolvedValue(mockSites);
      ...
      expect(mockListSitesWithStats).toHaveBeenCalledWith(orgId);
    });

    it('should return empty array when no sites', async () => {
      mockListSitesWithStats.mockResolvedValue([]);
  ...
  describe('get', () => {
    it('should get site by ID', async () => {
      mockGetSite.mockResolvedValue(baseMockSite);
      ...
      expect(result).toEqual(baseMockSite);
    ...
    it('should throw NOT_FOUND when site does not exist', async () => {
      ...
      await expect(caller.get({ organizationId: orgId, siteId })).rejects.toThrow(TRPCError);

      await expect(caller.get({ organizationId: orgId, siteId })).rejects.toMatchObject({
        code: 'NOT_FOUND',
  ...
  describe('create', () => {
    it('should create site when user is admin', async () => {
      mockGetUserRoleInOrg.mockResolvedValue('admin');
      mockCreateSite.mockResolvedValue(baseMockSite);
  ...
  describe('delete', () => {
    it('should delete site when user is admin', async () => {
      mockGetUserRoleInOrg.mockResolvedValue('admin');
      mockDeleteSite.mockResolvedValue(baseMockSite);
    ...
    it('should throw FORBIDDEN when non-admin tries to delete', async () => {
      ...
      await expect(caller.delete({ organizationId: orgId, siteId })).rejects.toThrow(TRPCError);

      await expect(caller.delete({ organizationId: orgId, siteId })).rejects.toMatchObject({
        code: 'FORBIDDEN',
