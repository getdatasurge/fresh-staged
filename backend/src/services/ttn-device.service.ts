 ...
 */
export async function getTTNConnection(
	organizationId: string,
): Promise<TtnConnection | null> {
 ...
 */
async function getTTNConfigForOrg(
	organizationId: string,
 ...
 */
export async function listTTNDevices(
	organizationId: string,
...
export async function provisionTTNDevice(
	...
	try {
		...
		if (error instanceof TTNApiError) {
			throw new TTNProvisioningError(
				`Failed to provision device in TTN: ${error.message}`,
			)
		}
		throw error
	}
