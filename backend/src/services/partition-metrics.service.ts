import { checkPartitionHealth, getPartitionMetrics } from './partition.service.js';
...
export async function getPartitionMetricsSnapshot(): Promise<PartitionMetricsSnapshot> {
  const [metrics, health] = await Promise.all([getPartitionMetrics(), checkPartitionHealth()]);
 ...
 */
export function formatPrometheusMetrics(snapshot: PartitionMetricsSnapshot): string {
  const lines: string[] = [];
  ...
  lines.push('# HELP sensor_readings_total_rows Total rows across all partitions');
  lines.push('# TYPE sensor_readings_total_rows gauge');
  lines.push(`sensor_readings_total_rows ${snapshot.total_rows}`);
  lines.push('');

  lines.push('# HELP sensor_readings_total_size_bytes Total size in bytes across all partitions');
  ...
  lines.push('# HELP sensor_readings_future_partition_count Number of future partitions');
  lines.push('# TYPE sensor_readings_future_partition_count gauge');
  lines.push(`sensor_readings_future_partition_count ${snapshot.future_partition_count}`);
  lines.push('');
  ...
  lines.push(`sensor_readings_default_partition_rows ${snapshot.default_partition_rows}`);
  lines.push('');

  lines.push('# HELP sensor_readings_partition_healthy Overall partition health status');
  ...
  for (const partition of snapshot.partitions) {
    lines.push(`sensor_readings_partition_rows{partition="${partition.name}"} ${partition.rows}`);
    lines.push(
