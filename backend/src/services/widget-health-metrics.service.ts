 ...
 */
export class WidgetHealthMetricsService {
  static trackHealthChange(event: Omit<WidgetHealthEvent, 'eventType' | 'timestamp'>): void {
    ...
    if (event.previousStatus) {
      orgCounters.counters[event.previousStatus] = Math.max(
        0,
        orgCounters.counters[event.previousStatus] - 1,
      );
    }
    ...
    if (shouldFlush) {
      void this.flushHealthMetrics(event.orgId);
    }
  }
   ...
   */
  static getHealthDistribution(orgId: string): Record<WidgetHealthStatus, number> {
   ...
   */
  static getBufferedEvents(orgId: string): WidgetHealthEvent[] {
   ...
   */
  static async flushHealthMetrics(orgId: string): Promise<void> {
    ...
    for (const event of events) {
      await AuditService.logEvent({
        ...
        severity: this.getSeverityForStatus(event.currentStatus),
        title: `Widget Health Changed: ${event.widgetId}`,
        organizationId: event.orgId,
        unitId: event.entityType === 'unit' ? event.entityId : null,
        siteId: event.entityType === 'site' ? event.entityId : null,
        actorId: 'system', // Widget health events are system-generated
        actorType: 'system',
        eventData: {
   ...
   */
  static getSeverityForStatus(status: WidgetHealthStatus): string {
    switch (status) {
      case 'error':
      case 'offline':
      case 'decoder_error':
   ...
   */
  static getFailuresByLayer(orgId: string): Record<FailingLayer, number> {
   ...
   */
  static hasCriticalIssues(orgId: string): boolean {
   ...
   */
  static resetOrgCounters(orgId: string): void {
    healthCountersMap.set(orgId, initOrgCounters(orgId));
  }
