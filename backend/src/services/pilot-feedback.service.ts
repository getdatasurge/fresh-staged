...
import {
	pilotFeedback,
	type PilotFeedback,
} from '../db/schema/pilot-feedback.js'

/**
 * Upsert pilot feedback for an organization
 */
export async function upsertFeedback(data: any): Promise<PilotFeedback> {
	const [result] = await db
		.insert(pilotFeedback)
		.values(data as any)
		.onConflictDoUpdate({
			target: [
				pilotFeedback.organizationId,
				pilotFeedback.siteId,
				pilotFeedback.weekStart,
			],
			set: {
