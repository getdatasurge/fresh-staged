 ...
 */
export async function listAlerts(organizationId: string, params: AlertQuery): Promise<Alert[]> {
  const { unitId, status, severity, start, end, limit = 100, offset = 0 } = params;
 ...
 */
export async function getAlert(alertId: string, organizationId: string): Promise<Alert | null> {
...
export async function acknowledgeAlert(
  ...
  const [updated] = await db
    ...
    .set({
      ...
      metadata: notes ? JSON.stringify({ acknowledgementNotes: notes }) : alert.metadata,
      updatedAt: new Date(),
    })
