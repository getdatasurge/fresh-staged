...
import { areas, sites, units, type InsertUnit, type Unit } from '../db/schema/hierarchy.js';
...
export async function createUnit(
  ...
  siteId: string,
  organizationId: string,
  data: Omit<InsertUnit, 'id' | 'areaId' | 'createdAt' | 'updatedAt' | 'isActive'>,
): Promise<Unit | null> {
  // Verify full hierarchy first
  if (!(await verifyAreaAccess(areaId, siteId, organizationId))) {
    return null; // Area not found or not in org
...
export async function restoreUnit(
  ...
  const [unit] = await db
    ...
    .where(and(eq(units.id, unitId), eq(units.areaId, areaId), eq(units.isActive, false)))
    .returning();
...
export async function getUnitWithHierarchy(
  ...
  const [result] = await db
    ...
    .innerJoin(sites, eq(areas.siteId, sites.id))
    .where(
      and(eq(units.id, unitId), eq(sites.organizationId, organizationId), eq(units.isActive, true)),
    )
    .limit(1);
