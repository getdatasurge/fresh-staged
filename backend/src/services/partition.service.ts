 ...
 */
function parsePartitionBound(partitionName: string, boundExpression: string): PartitionBound {
...
export async function createFuturePartitions(bufferMonths: number = 3): Promise<string[]> {
  ...
  for (let i = 0; i <= bufferMonths; i++) {
    const targetMonth = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth() + i, 1));
...
export async function enforceRetentionPolicy(
  ...
  const cutoffDate = new Date(
    Date.UTC(new Date().getUTCFullYear(), new Date().getUTCMonth() - retentionMonths, 1),
  );
  ...
  for (const partition of partitions) {
    if (partition.toValue < cutoffDate) {
      ...
      await db.execute(sql.raw(`DROP TABLE IF EXISTS ${partition.partitionName}`));
...
export async function checkPartitionHealth(): Promise<{
  ...
  `);
  const defaultPartitionRows = parseInt((defaultPartitionResult.rows[0] as any)?.count || '0', 10);
