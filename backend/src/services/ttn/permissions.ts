...
export class TtnPermissionService {
   ...
   */
  static async validateMainUserApiKey(
    apiKey: string,
    requestId: string
    ...
    try {
      ...
      // Reject scoped keys
      if (entityType === "application_ids" || entityType === "organization_ids" || entityType === "gateway_ids") {
        return {
          success: false,
          user_id: userId,
  ...
  static async fetchTtnRights(
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 401) return { success: false, error: "Invalid API key", statusCode: 401 };
        if (response.status === 403) return { success: false, error: "Access denied", statusCode: 403 };
        if (response.status === 404) return { success: false, error: "Application not found", statusCode: 404 };
        
        return { success: false, error: `TTN API error (${response.status})`, hint: errorText, statusCode: response.status };
      }
  ...
  static async validateAndAnalyzePermissions(
    ...
    apiKey: string,
    requestId: string
  ): Promise<{ success: boolean; report?: PermissionReport; error?: string; hint?: string; statusCode?: number }> {
