...
export class TtnWebhookService {
  ...
  static async ensureWebhook(
    ...
    const response = await TtnClient.fetch(endpoint, apiKey, {
      ...
      body: JSON.stringify({
        webhook: body,
        field_mask: {
          paths: ['base_url', 'format', 'headers', 'uplink_message', 'join_accept'],
        },
      }),
    });
   ...
   */
  static async updateWebhook(organizationId: string, url: string, events: string[]) {
    const record = await db.query.ttnConnections.findFirst({
      where: eq(ttnConnections.organizationId, organizationId),
    });
    ...
    if (events.includes('uplink_message')) body.uplink_message = { path: '/uplink' };
  ...
  static async regenerateWebhookSecret(
    ...
    // Update in TTN
    const webhookUrl =
      record.ttnWebhookUrl || `${process.env.API_BASE_URL}/api/webhooks/ttn/${organizationId}`;
    const body: any = {
      ids: { webhook_id: webhookId },
      base_url: webhookUrl,
      format: 'json',
