...
export async function listSitesWithStats(
	...
	return db
		...
		.where(
			and(eq(sites.organizationId, organizationId), eq(sites.isActive, true)),
		)
		.groupBy(sites.id)
		.orderBy(sites.name)
}
...
export async function createSite(
	...
	data: Omit<
		InsertSite,
		'id' | 'organizationId' | 'createdAt' | 'updatedAt' | 'isActive'
	>,
): Promise<Site> {
	const [site] = await db
		.insert(sites)
...
export async function updateSite(
	...
	organizationId: string,
	data: Partial<
		Omit<InsertSite, 'id' | 'organizationId' | 'createdAt' | 'updatedAt'>
	>,
): Promise<Site | null> {
	const [site] = await db
		.update(sites)
