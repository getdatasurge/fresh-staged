...
export const alertRulesRouter = router({
  ...
  upsert: orgProcedure.input(UpsertRulesInput).mutation(async ({ ctx, input }) => {
    const scope = {
      organizationId: ctx.user.organizationId,
      siteId: input.siteId,
      unitId: input.unitId,
    ...
    const rules = await alertRulesService.upsertAlertRules(scope, input.data as any);
    return rules;
  }),

  delete: orgProcedure.input(DeleteRulesInput).mutation(async ({ ctx, input }) => {
    const scope = {
      organizationId: ctx.user.organizationId,
      siteId: input.siteId,
      unitId: input.unitId,
  ...
  clearField: orgProcedure.input(ClearFieldInput).mutation(async ({ ctx, input }) => {
