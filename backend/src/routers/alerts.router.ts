...
export const alertsRouter = router({
		...
		.mutation(async ({ ctx, input }) => {
			...
			if (!['staff', 'manager', 'admin', 'owner'].includes(ctx.user.role)) {
				throw new TRPCError({
					code: 'FORBIDDEN',
					message:
						'Only staff, managers, admins, and owners can acknowledge alerts',
				})
			}
		...
		.mutation(async ({ ctx, input }) => {
			...
			if (!['staff', 'manager', 'admin', 'owner'].includes(ctx.user.role)) {
				throw new TRPCError({
					code: 'FORBIDDEN',
					message:
						'Only staff, managers, admins, and owners can resolve alerts',
				})
			}
	...
	listByOrg: orgProcedure
		...
		.query(async ({ ctx, input }) => {
			...
			const alerts = await alertService.listAlertsWithHierarchy(
				ctx.user.organizationId,
				{
					status: input.status,
					severity: input.severity,
