...
export const sitesRouter = router({
  ...
  list: orgProcedure
    .input(OrgInput)
    .output(SitesWithStatsListSchema)
    .query(async ({ ctx }) => {
      const sites = await siteService.listSitesWithStats(ctx.user.organizationId);
      return sites;
    }),
   ...
   * Equivalent to: POST /api/orgs/:organizationId/sites/:siteId/restore
   *
  ...
  restore: orgProcedure
    ...
    .output(SiteSchema)
    .mutation(async ({ ctx, input }) => {
      // Role check - only admins and owners can restore sites
      if (!['admin', 'owner'].includes(ctx.user.role)) {
        throw new TRPCError({
          code: 'FORBIDDEN',
          message: 'Only admins and owners can restore sites',
        });
      }

      const site = await siteService.restoreSite(input.siteId, ctx.user.organizationId);
   ...
   * Equivalent to: DELETE /api/orgs/:organizationId/sites/:siteId/permanent
   *
  ...
  permanentlyDelete: orgProcedure
    ...
    .output(z.void())
    .mutation(async ({ ctx, input }) => {
      // Role check - only admins and owners can permanently delete sites
      if (!['admin', 'owner'].includes(ctx.user.role)) {
        throw new TRPCError({
          code: 'FORBIDDEN',
          message: 'Only admins and owners can permanently delete sites',
        });
      }

      const site = await siteService.permanentlyDeleteSite(input.siteId, ctx.user.organizationId);
