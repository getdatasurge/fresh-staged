			...
			const stats = await Promise.all(
				orgs.map(async org => {
					...
					const [siteCountResult] = await db
						...
						.from(sites)
						.where(
							and(eq(sites.organizationId, org.id), isNull(sites.deletedAt)),
						)
		...
		.query(async ({ input }) => {
			...
			return results.map(entry => {
				...
				return {
					...
					impersonatedUserId:
						typeof eventData.impersonatedUserId === 'string'
							? eventData.impersonatedUserId
							: null,
					targetType:
						typeof eventData.targetType === 'string'
							? eventData.targetType
							: null,
					targetId:
						typeof eventData.targetId === 'string' ? eventData.targetId : null,
					targetOrgId:
						typeof eventData.targetOrgId === 'string'
							? eventData.targetOrgId
							: null,
