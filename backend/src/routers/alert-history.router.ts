...
const CreateHistoryInput = z.object({
  ...
  action: z.string(),
  changes: z.object({}).catchall(z.any()),
  oldValues: z.object({}).catchall(z.any()).optional(),
});

export const alertHistoryRouter = router({
  get: orgProcedure.input(GetHistoryInput).query(async ({ ctx, input }) => {
    const scope = {
  ...
  create: orgProcedure.input(CreateHistoryInput).mutation(async ({ ctx, input }) => {
