...
function formatCsv(
  ...
  const csvRows = readings.map(
    (r) => `${r.timestamp.toISOString()},${r.unitName},${r.temperature},${r.humidity ?? ''}`,
  );
  ...
  export: orgProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      try {
        ...
        const unitIds = await getUnitIdsForQuery(organizationId, siteId, unitId);

        // If no units found, return empty report
        if (unitIds.length === 0) {
          const content =
            format === 'csv' ? formatCsv([]) : formatHtml(reportType, startDate, endDate, []);
