...
export const inspectorRouter = router({
   ...
   */
  getOrgData: orgProcedure
    .input(getOrgDataSchema)
    .query(async ({ ctx, input }) => {
      ...
      const filteredSites = input.allowedSiteIds && input.allowedSiteIds.length > 0
   ...
   */
  getUnits: orgProcedure
    .input(getUnitsSchema)
    .query(async ({ ctx, input }) => {
   ...
   */
  getInspectionData: orgProcedure
    .input(getInspectionDataSchema)
    .query(async ({ ctx, input }) => {
      ...
      return {
        ...
        alerts: alertsData.map(a => ({
          ...
          acknowledgment_notes: a.metadata ? JSON.parse(a.metadata as string)?.acknowledgmentNotes : undefined,
        })),
        ...
        monitoringGaps: gapsData
          ...
          .map(g => {
            ...
            return {
              id: g.id,
              unit_id: g.unitId || '',
              gap_type: g.eventType === 'missed_manual_log'
                ? 'Missed Manual Log'
