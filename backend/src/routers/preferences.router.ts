...
export const preferencesRouter = router({
	...
	updateDigest: protectedProcedure
		...
		.mutation(async ({ ctx, input }) => {
			...
			if (input.digestDaily !== undefined)
				updates.digestDaily = input.digestDaily
			if (input.digestWeekly !== undefined)
				updates.digestWeekly = input.digestWeekly
			if (input.digestDailyTime !== undefined)
				updates.digestDailyTime = input.digestDailyTime
			if (input.digestSiteIds !== undefined) {
				updates.digestSiteIds = input.digestSiteIds
					? JSON.stringify(input.digestSiteIds)
					: null
			}
