...
const HealthStatusSchema = z.enum(['healthy', 'degraded', 'unhealthy', 'unknown', 'checking']);
const HealthCategorySchema = z.enum(['edge_function', 'database', 'ttn', 'external', 'cache']);
      ...
      const checks = [
        {
          ...
          checkedAt: new Date().toISOString(),
          skipped: false,
          details: dbStatus === 'healthy' ? { latency_ms: 0 } : { message: 'Database unreachable' },
        },
        {
