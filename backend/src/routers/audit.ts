...
const LogEventSchema = z.object({
  eventType: z.string(),
  category: z
    .enum(['alert', 'compliance', 'settings', 'user_action', 'system'])
    .default('user_action'),
  ...
  eventData: z.object({}).catchall(z.any()).optional(),
  impersonationSessionId: z.string().optional().nullable(),
  actingAdminId: z.string().optional().nullable(),
});

export const auditRouter = router({
  logEvent: orgProcedure.input(LogEventSchema).mutation(async ({ input, ctx }) => {
