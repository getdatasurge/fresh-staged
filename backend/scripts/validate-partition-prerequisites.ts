    ...
    if (minDate && maxDate) {
      ...
      log({
        ...
        details: {
          ...
          lastPartitionMonth: new Date(maxMonth.getTime() - 24 * 60 * 60 * 1000)
            .toISOString()
            .substring(0, 7),
          partitionCount,
        },
...
async function testDrizzleORMWithPartitions() {
  try {
    ...
    await db.execute(sql.raw(`DROP TABLE IF EXISTS ${testTableName} CASCADE`));
    ...
    if (partitionPruningWorks) {
      ...
      log({
        check: 'Drizzle ORM Partition Compatibility',
        status: 'WARN',
        message: 'Partition pruning may not be optimal. Review EXPLAIN ANALYZE output.',
...
async function analyzeTrafficPatterns() {
  try {
    ...
    const minTrafficHour = hourlyStats.reduce((min, curr) => (curr.count < min.count ? curr : min));
    const maxTrafficHour = hourlyStats.reduce((max, curr) => (curr.count > max.count ? curr : max));

    const avgCount = hourlyStats.reduce((sum, stat) => sum + stat.count, 0) / hourlyStats.length;
...
async function generateValidationReport() {
  ...
  if (statusCounts.FAIL > 0) {
    ...
    console.log('✅ All critical checks passed. Migration prerequisites validated.\n');
  }
