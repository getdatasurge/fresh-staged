...
- [ ] **Fix one documentation gap (or skip if none)**: Read {{AUTORUN_FOLDER}}/LOOP*{{LOOP_NUMBER}}\_PLAN.md. If the file doesn't exist OR contains no items with status exactly `PENDING`, mark this task complete without changes. Otherwise, find an item with status exactly `PENDING`, implement the fix in the project's README.md, mark as IMPLEMENTED in the plan, and log to {{AUTORUN_FOLDER}}/USAGE_LOG*{{AGENT_NAME}}\_{{DATE}}.md. Only fix ONE gap per task.
...
- [ ] Change matches the proposed fix from LOOP\_{{LOOP_NUMBER}}\_PLAN.md
