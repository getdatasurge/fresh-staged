...
- [ ] **Evaluate one candidate (or skip if empty)**: Read {{AUTORUN_FOLDER}}/LOOP*{{LOOP_NUMBER}}\_CANDIDATES.md. If it contains no findings OR all findings have already been evaluated in LOOP*{{LOOP_NUMBER}}_PLAN.md, mark this task complete without changes. Otherwise, pick one unevaluated candidate, assess risk/benefit, and append to {{AUTORUN_FOLDER}}/LOOP_{{LOOP_NUMBER}}\_PLAN.md.
